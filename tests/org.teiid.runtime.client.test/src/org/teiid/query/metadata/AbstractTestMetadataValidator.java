...
public abstract class AbstractTestMetadataValidator extends AbstractTest {

    public final SystemFunctionManager SFM;
    ...
    private Map<String, Datatype> getDataTypes() {
        return SystemMetadata.getInstance(getTeiidVersion()).getRuntimeTypeMap();
    }
    ...
    private ModelMetaData buildModel(String modelName, boolean physical, VDBMetaData vdb, MetadataStore store, String ddl)
        throws Exception {
        ...
        MetadataFactory mf = new MetadataFactory(getTeiidVersion(), "myVDB", 1, modelName, getDataTypes(), new Properties(), ddl);
        mf.setParser(getQueryParser());
        mf.setBuiltinDataTypes(SystemMetadata.getInstance(getTeiidVersion()).getSystemStore().getDatatypes());
        mf.getSchema().setPhysical(physical);
        mf.parse(new StringReader(ddl));
        mf.mergeInto(store);
    ...
    private String printError(ValidatorReport report) {
        ...
        for (ValidatorFailure v : report.getItems()) {
            if (v.getStatus() == IValidatorFailure.VFStatus.ERROR) {
                sb.append(v);
                sb.append("\n");
            }
        }
    ...
    public void testMinimalDataNoColumns() throws Exception {
        ...
        ValidatorReport report = new ValidatorReport();
        MetadataValidator mv = newMetadataValidator();
        mv.new MinimalMetadata().execute(vdb, store, report, newMetadataValidator());
        assertTrue(printError(report), report.hasItems());
    }
    ...
    public void testResolveMetadata() throws Exception {
        ...
        ValidatorReport report = new ValidatorReport();
        MetadataValidator mv = newMetadataValidator();
        mv.new ResolveQueryPlans().execute(vdb, store, report, newMetadataValidator());
        assertTrue(printError(report), report.hasItems());
    }
    ...
    public void testProcWithMultipleReturn() throws Exception {
        ...
        buildModel("pm1", true, this.vdb, this.store, ddl);
        buildTransformationMetadata();
        ValidatorReport report = newMetadataValidator().validate(vdb, store);
        assertTrue(printError(report), report.hasItems());
    }
    ...
    public void testProcMetadataValidationError() throws Exception {
        ...
        MetadataValidator mv = newMetadataValidator();
        mv.new ResolveQueryPlans().execute(vdb, store, report, newMetadataValidator());
        assertEquals("Element x.e2 of x is neither nullable nor has a default value. A value must be specified in the insert.",
                     report.getItems().iterator().next().toString());
    }
    ...
    public void testResolveMetadataError() throws Exception {
        buildModel("vm1",
                   ...
                   this.vdb,
                   this.store,
                   "create view g1 (e1 integer, e2 varchar(12)) AS select * from pm1.g1; create view g2 AS select * from pm1.g1;");
        buildTransformationMetadata();
        ValidatorReport report = new ValidatorReport();
        MetadataValidator mv = newMetadataValidator();
        mv.new ResolveQueryPlans().execute(vdb, store, report, newMetadataValidator());
    ...
    public void testCrossReferenceFK() throws Exception {
        ...
        ValidatorReport report = new ValidatorReport();
        report = newMetadataValidator().validate(this.vdb, this.store);
        assertFalse(printError(report), report.hasItems());
        ...
        assertEquals("g1e1",
                     this.store.getSchema("pm2").getTable("G2").getForeignKeys().get(0).getReferenceKey().getColumns().get(0).getName());
    }
    ...
    public void testSkipDocumentModel() throws Exception {
        ...
        MetadataFactory mf = new MetadataFactory(getTeiidVersion(), "myVDB", 1, "xmlstuff", getDataTypes(), new Properties(),
                                                 null);
        ...
        ValidatorReport report = newMetadataValidator().validate(this.vdb, this.store);
        assertFalse(printError(report), report.hasItems());
    }
    ...
    @Test
    public void testReturnResolving() throws Exception {
        String ddl = "create procedure proc (x integer) returns string as return x;\n";
        helpTest(ddl, false);
    }

    @Test
    public void testReturnResolving1() throws Exception {
    ...
    public void testViewKeys() throws Exception {
        buildModel("phy1",
                   ...
                   this.vdb,
                   this.store,
                   "CREATE FOREIGN TABLE t1 ( col1 string, col2 integer ) options (updatable true)");
        buildModel("phy2",
                   true,
                   this.vdb,
                   this.store,
                   "CREATE FOREIGN TABLE t2 ( col1 string, col2 integer ) options (updatable true)");
        buildModel("view1",
                   false,
                   this.vdb,
                   this.store,
                   "CREATE view vw_t1 ( col1 string, col2 integer primary key, foreign key (col2) references vw_t2 (col2) ) options (updatable true) as select * from t1;"
    ...
    public void testResolvingOrder() throws Exception {
        buildModel("phy1",
                   ...
                   this.vdb,
                   this.store,
                   "CREATE FOREIGN TABLE t1 ( col1 string, col2 integer ) options (updatable true); CREATE view a as select * from t1;");
