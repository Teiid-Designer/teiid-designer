...
//        suite.addTest(new TestVdbExecutionValidator("testPathExist"));//$NON-NLS-1$
    ...
    public VdbEditingContext helpGetUnSavedEditingContext() throws Exception {
        FakeVdbEditingContext context = new FakeVdbEditingContext();
        context.setSaveRequired(true);
        File defFile = new File(SmartTestSuite.getTestScratchPath()+"FakeVdb.DEF"); //$NON-NLS-1$
        //defFile.createNewFile();
        context.setDefnFile(defFile);
        return context;
    }
    
    public VdbEditingContext helpGetStaleEditingContext() throws Exception {
        FakeVdbEditingContext context = new FakeVdbEditingContext();
        context.setSaveRequired(false);
        File defFile = new File(SmartTestSuite.getTestScratchPath()+"FakeVdb.DEF"); //$NON-NLS-1$
        //defFile.createNewFile();
        context.setDefnFile(defFile);
    ...
    public VDBDefn helpGetVdbDefnWithConnectorBindingss() {
        FakeVdbDefn defn = new FakeVdbDefn();
        defn.setModelBinding(PHYSICAL_MODEL_NAME1, "binding1"); //$NON-NLS-1$
        defn.setModelBinding(PHYSICAL_MODEL_NAME2, "binding2"); //$NON-NLS-1$
        defn.setModelBinding(PHYSICAL_MODEL_NAME3, "binding3"); //$NON-NLS-1$
        return defn;
    }

    public VDBDefn helpGetVdbDefnWithMissingConnectorBindings() {
        FakeVdbDefn defn = new FakeVdbDefn();
        defn.setModelBinding(PHYSICAL_MODEL_NAME1, "binding1"); //$NON-NLS-1$
        defn.setModelBinding(PHYSICAL_MODEL_NAME2, "binding2"); //$NON-NLS-1$
        return defn;
    ...
    public void testUnSaveVdb() throws Exception {
        FakeVdbEditingContext context = (FakeVdbEditingContext) helpGetUnSavedEditingContext();
        VirtualDatabase database = helpGetVirtualDatabaseWithPhysicalModels();
        context.setDatabase(database);
        VdbExecutionValidatorImpl validator = new VdbExecutionValidatorImpl();
        IStatus status = validator.validateVdb(context);
        assertEquals(IStatus.ERROR, status.getSeverity());        
    }
...
//    }
    ...
    public void testVdbWithBindings() {
        VirtualDatabase database = helpGetVirtualDatabaseWithPhysicalModels();
        VDBDefn vdbDefn = helpGetVdbDefnWithConnectorBindingss();
        VdbExecutionValidatorImpl validator = new VdbExecutionValidatorImpl();
        IStatus status = validator.validateVdbModels(database, vdbDefn);
        assertEquals(IStatus.OK , status.getSeverity());
    }

    public void testVdbWithMissingBindings() {
        VirtualDatabase database = helpGetVirtualDatabaseWithPhysicalModels();
        VDBDefn vdbDefn = helpGetVdbDefnWithMissingConnectorBindings();
        VdbExecutionValidatorImpl validator = new VdbExecutionValidatorImpl();
        IStatus status = validator.validateVdbModels(database, vdbDefn);
        assertEquals(IStatus.ERROR , status.getSeverity());
    }
