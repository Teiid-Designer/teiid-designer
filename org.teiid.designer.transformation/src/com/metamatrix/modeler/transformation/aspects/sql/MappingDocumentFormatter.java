...
public class MappingDocumentFormatter {
     ...
     * 
     */
    public static MappingDocumentFormatter create( final XmlDocument xmlDoc, final MappingContext mappingContext ) {
        ...
        if ( treeMappingRoots != null && treeMappingRoots.size() != 0 ) {
            ...
            if ( mappingClassSet != null ) {
                final MappingDocumentFormatter formatter = new MappingDocumentFormatter(xmlDoc,treeMappingRoots,mappingClassSet,mappingContext);
                return formatter;
            }
        }
        return null;
     ...
     * 
     */
    public MappingDocumentFormatter( final XmlDocument xmlDoc, final List treeMappingRoots,
                                     final MappingClassSet mappingClassSet, final MappingContext mappingContext ) {
        super();
        ArgCheck.isNotNull(xmlDoc);
...
//        final SchemaIncludeMap schemaIncludeMap = new SchemaIncludeMap(moe);
        ...
        processChildren(xmlRootElement.getContainers(),rootNode,nsContext,elementInfo);
        processChildren(xmlRootElement.getProcessingInstructions(),rootNode,nsContext,elementInfo);
        processChildren(xmlRootElement.getElements(),rootNode,nsContext,elementInfo);
     ...
     * @param parent parent MappingNode of the MappingNode to be created in this method call;
     * this should never be null
     * @param context the namespace context for this entity ...
     */
    protected void createMapping( final XmlDocumentEntity entity, final MappingNode parent,
                                  final NamespaceContext namespaceContext, final MappingContext mappingContext, 
        ...
        switch ( classifierId ) {
            case XmlDocumentPackage.XML_ELEMENT:
                ...
                processChildren(element.getContainers(),entityMappingNode,entityNamespaceContext,entityInfo);
                processChildren(element.getProcessingInstructions(),entityMappingNode,entityNamespaceContext,entityInfo);
                processChildren(element.getElements(),entityMappingNode,entityNamespaceContext,entityInfo);
                break;
    ...
    protected void processNamespaces( final List children, final MappingNode parentMappingNode,
                                    final NamespaceContext namespaceContext, final ElementInfo parentInfo) {
    ...
    protected void processChildren( final List children, final MappingNode parentMappingNode,
                                    final NamespaceContext namespaceContext, final ElementInfo parentInfo) {
        final Iterator iter = children.iterator();
        while (iter.hasNext()) {
            final Object child = iter.next();
            if ( child instanceof XmlDocumentEntity ) {
                createMapping((XmlDocumentEntity)child,parentMappingNode,namespaceContext,mappingContext,parentInfo);
            }
        }
    }
     ...
     * @param rootElement
     */
    protected MappingDocument createDocumentNode(final XmlRoot xmlRoot, final ElementInfo rootElementInfo, final MappingContext mappingContext ) {

        // Set the SOAP encoding information ...
        this.includeSoapDefaultEncoding = xmlDoc.getSoapEncoding().getValue() == SoapEncoding.DEFAULT;
        MappingDocument doc = new MappingDocument(xmlDoc.getEncoding(), xmlDoc.isFormatted());
        MappingElement rootNode = processElementNode(xmlRoot, rootElementInfo, mappingContext);
     ...
     * @param rootElement
     */
    protected MappingNode createElementNode(MappingBaseNode parent, final XmlElement element, final ElementInfo elementInfo, final MappingContext mappingContext ) {
        MappingCriteriaNode criteria = createCriteriaNode(element);
        if ( parent != null && parent instanceof MappingChoiceNode && criteria != null) {
            parent = ((MappingChoiceNode)parent).addCriteriaNode(criteria);
        }
        
        MappingElement node = processElementNode(element,elementInfo,mappingContext);
        if (parent != null){
    ...
    MappingCriteriaNode createCriteriaNode(ChoiceOption element) {
        ...
        boolean defalt = (element.getDefaultFor() != null);
     ...
     * @param rootElement
     */
    protected MappingElement processElementNode(final XmlElement element, final ElementInfo elementInfo, final MappingContext mappingContext ) {
        MappingElement node = null;
        
        String name = element.getName();
        String nsPrefix = getNamespacePrefix(element,elementInfo.getNamespaceContext(),mappingContext);
        ...
        if ( this.includeSoapDefaultEncoding ) {
            ...
            String soapArrayType = getSoapArrayType(element,elementInfo.getNamespaceContext(),mappingContext);
    ...
    String getRecursionMappingClass(XmlElement element) {
        ...
        if (rootMappingClass != null) {
            return getFullName(rootMappingClass);
        }
        return null;
    }
    ...
    String getRecursionCriteria(XmlElement element) {
        ...
        if ( mappingClass != null && mappingClass.isRecursionAllowed() && mappingClass.isRecursive() ) {
            ...
            if (!StringUtil.isEmpty(criteria)) {
                criteria = StringUtil.replaceAll(criteria, getFullName(mappingClass), getRecursionMappingClass(element));
            }            
        }
        return criteria;        
    }
    ...
    boolean throwExceptionOnRecursionLimit(XmlElement element) {
        final MappingClass mappingClass = this.helper.getMappingClass(element);
        if ( mappingClass != null && mappingClass.isRecursionAllowed() && mappingClass.isRecursive() ) {
            return (mappingClass.getRecursionLimitErrorMode().getValue() == RecursionErrorMode.THROW);
        }
    ...
    String getCriteria(ChoiceOption element) {
        ...
        if (choiceCriteria != null && choiceCriteria.indexOf(UUID.PROTOCOL) >= 0) {
            // Defect 23725 - changed call to get the element's Container object. CANNOT ASSUME RESOURCE SET is CONTAINER
    ...
    String getBuitInType(XmlElement element) {
        final MappingClassColumn mappingClassColumn = this.helper.getMappingClassColumn(element);
        if (mappingClassColumn != null) {
            SqlColumnAspect columnAspect = (SqlColumnAspect)com.metamatrix.modeler.core.metamodel.aspect.sql.SqlAspectHelper.getSqlAspect(mappingClassColumn);

            EObject dataType = columnAspect.getDatatype(mappingClassColumn);

            final DatatypeManager dtm = ModelerCore.getBuiltInTypesManager();
            
            Assertion.isNotNull(dtm);
    ...
    String getDefaultValue(XmlValueHolder valueHolder) {
        ...
        if ( value != null ) {
            ...
            if (valueType.getValue() == ValueType.DEFAULT) {
                return value;
            }            
        }    
        return null;
    ...
    String getFixedValue(XmlValueHolder valueHolder) {
        ...
        if ( value != null ) {
            final ValueType valueType = valueHolder.getValueType();
            if (valueType.getValue() == ValueType.FIXED) {
                return value;
            }            
        }    
        return null;
     ...
     * @param rootElement
     */
    protected MappingNode createSequenceNode(MappingBaseNode parent, final XmlContainerNode compositor, final ElementInfo elementInfo ) {
        MappingCriteriaNode criteria = createCriteriaNode(compositor);
        if ( parent != null && parent instanceof MappingChoiceNode && criteria != null) {
            parent = ((MappingChoiceNode)parent).addCriteriaNode(criteria);
        }
    ...
    protected MappingNode createChoiceNode(MappingBaseNode parent, final XmlChoice choice, final ElementInfo elementInfo ) {
        MappingCriteriaNode criteria = createCriteriaNode(choice);
        if ( parent != null && parent instanceof MappingChoiceNode && criteria != null) {
            parent = ((MappingChoiceNode)parent).addCriteriaNode(criteria);
        }
        
        MappingChoiceNode choiceNode = new MappingChoiceNode(choice.getDefaultErrorMode().getValue() == ChoiceErrorMode.THROW);        
    ...
    private String getStagingTable(final XmlDocumentEntity compositor) {
        ...
        if ( tempGroups != null && tempGroups.length != 0) {
            for (int i = 0; i < tempGroups.length; i++) {
                final StagingTable st = tempGroups[i];
    ...
    protected void createAttributeNode(final MappingElement parent, final XmlAttribute element, final ElementInfo elementInfo, final MappingContext mappingContext ) {
        String name = element.getName();
        String nsPrefix = getNamespacePrefix(element,elementInfo.getNamespaceContext(),mappingContext);
        ...
        else if (name != null && nsPrefix != null && nsPrefix.equalsIgnoreCase(MappingNodeConstants.NAMESPACE_DECLARATION_ATTRIBUTE_NAMESPACE)) {
    ...
    protected MappingNode createCommentNode(final MappingElement parent, final XmlComment comment, final ElementInfo elementInfo ) {
     ...
     * @param rootElement
     */
    protected void createNamespaceAttribute(final MappingElement parent, final XmlNamespace ns, final ElementInfo elementInfo ) {
     ...
     * @param rootElement
     */
    protected MappingAttribute createXsiTypeAttribute(MappingElement parent, final ElementInfo elementInfo ) {
        ...
        String prefix = getSchemaInstanceNamespacePrefix(context);
     ...
     * @param rootElement
     */
    protected MappingNode createSoapArrayTypeAttribute(final MappingElement parent, final ElementInfo elementInfo) {
        ...
        NamespaceContext context = elementInfo.getNamespaceContext();
        String prefix = getSoapEncodingNamespacePrefix(context);
        Namespace namespace = getNamespace(prefix);

        MappingAttribute attribute = new MappingAttribute(Soap.ARRAY_TYPE_XML_ATTRIBUTE_NAME, namespace);
        attribute.setValue(value);
        attribute.setOptional(true);
        attribute.setAlwaysInclude(true);
     ...
     * @param context the namespace context from which namespace prefixes should be determined;
     * never null
     * @return the value of the "xsi:type" attribute; may be null if there is no type
     */
    protected String getXsiType( final XmlElement element, final NamespaceContext context, final MappingContext mappingContext) {
        // Get the type ...
        final XSDComponent xsdComponent = element.getXsdComponent();
        if ( xsdComponent == null ) {
            return null;
     ...
     */
    protected String getXsiTypeTextNormalization( final XmlDocumentNode node, final MappingContext mappingContext) {
        // Get the type ...
        ...
        if (typeDefn instanceof XSDSimpleTypeDefinition) {
            ...
            XSDWhiteSpaceFacet facet = simpleTypeDefn.getEffectiveWhiteSpaceFacet();
            if (facet != null) {
                XSDWhiteSpace whiteSpaceEnum = facet.getValue();
                if (whiteSpaceEnum != null) {
                    switch (whiteSpaceEnum.getValue()) {
                        case XSDWhiteSpace.PRESERVE: return MappingNodeConstants.NORMALIZE_TEXT_PRESERVE;
                        case XSDWhiteSpace.REPLACE:  return MappingNodeConstants.NORMALIZE_TEXT_REPLACE;
                        case XSDWhiteSpace.COLLAPSE: return MappingNodeConstants.NORMALIZE_TEXT_COLLAPSE;
                    }
                }
            }
        }
...
//        }
    ...
    /**
     * Utility to get a non-empty namespace prefix regardless of what the actual XmlNamespace has for
     * it's prefix.  This method does not actually change the XmlNamespace object.
     * @param ns
     * @return
     */
    static String getNamespacePrefix( final XmlNamespace ns ) {
        ...
        if ( prefix == null || prefix.trim().length() == 0 ) {
            prefix = XmlDocumentUtil.createXmlPrefixFromUri(ns.getUri());
        }
        return prefix;
    }
    ...
    static String getNamespacePrefix( final XmlDocumentNode elementOrAttribute, 
        ...
        XSDSchema startingSchema  = getTargetNamespaceSchema(xsdComponent, context, mappingContext);
    ...
    private static String getNamespacePrefix( XSDComponent xsdComponent, 
                                              final NamespaceContext context,
                                              final MappingContext mappingContext,  
        ...
        if ( namespace == null ) {
            ...
            if ( XsdUtil.isBuiltInDatatype(xsdComponent) ) {
                ...
                if ( !checkWhetherPrefixIsRequired || isPrefixRequired(xsdComponent,schema,uri) ) {
                    return prefix;
                }
                return null;
            }
        ...
        if ( namespace != null ) {
            ...
            if ( actualPrefix == null || actualPrefix.trim().length() == 0 ) {
                // This is the default target namespace declaration, so return null (see defect 13428)
                return null;
            }
    ...
    /**
     ...
     private static XSDSchema getTargetNamespaceSchema( XSDComponent xsdComponent, 
         ...
         if(schema == null && xsdComponent.eIsProxy()){
             ...
             if(schema == null){
                 Object componentURi = ModelerCore.getModelEditor().getUri(xsdComponent);
         ...
         if (xsdComponent instanceof XSDFeature) {
            ...
            if (resolvedComponent != null && resolvedComponent != xsdComponent) {
                ...
                if (StringUtil.isEmpty(resolvedTNS)) {
                    resolvedTNS = resolvedSchema.getTargetNamespace();
                }
                if (resolvedTNS != null && resolvedTNS.trim().length() != 0) {
                    // The referenced schema DOES have a target namespace, so this is the
                    ...
                    if (resolvedSchema != null && resolvedSchema.eIsProxy()) {
                        ...
                        if (proxyURI != null) {
                            XSDResourceImpl resource = (XSDResourceImpl)mappingContext.getResourceSet().getResource(proxyURI.trimFragment(), true);
                            if (resource != null) {
                ...
                // otherwise, the target namespace of the original schema will be used (if there is one)
            }
        }
    ...
    /**
     * Return the target namespace string for the specified XmlDocumentNode. If the XmlDocumentNode
     * has no XSDComponent reference or is determined to reference a global schema then null 
     * is returned.
     * @param element
     * @param context
     * @param mappingContext
    ...
    private static String getTargetNamespace( final XSDSchema startingSchema,
        ...
        final XSDSchema schema = getTargetNamespaceSchema(xsdComponent, context, mappingContext);
        String targetNS = ( schema == null ? null : schema.getTargetNamespace());

        // If we find a target namespace ...
        if (!StringUtil.isEmpty(targetNS)) {
            ...
            if (startingSchema != null && startingSchema.getTargetNamespace() == null) {
                ...
                if (hasMatchingIncludeDirective(schema, startingSchema)) {
                    // The starting schema is a chameleon schema that has been included in 
    ...
    private static boolean hasMatchingIncludeDirective(final XSDSchema schemaWithDirectives, final XSDSchema schemaToCheck) {
        if (schemaWithDirectives != null && schemaToCheck != null) {
            // Check all XSDInclude declarations for the specified schema ("schemaToCheck")
            for (Iterator iter = schemaWithDirectives.eContents().iterator(); iter.hasNext();) {
                ...
                if (content instanceof XSDInclude) {
                    ...
                    if (includeSchemaLocation  != null && includeSchemaLocation.equals(schemaToCheck.getSchemaLocation())) {
                        return true;
                    }
                } 
            }
     ...
     * <p>
     * If the XSD copmonent is an element and:
     * <ul>
     ...
     *  <li>is local, the value of the "form" attribute determines whether the element is qualified
     *      or unqualified.  If the local element has no "form" attribute, the value of the schema's 
     *      "elementFormDefault" attribute determines whether the element is qualified or unqualified</li>
     * </ul>
     * </p><p>
     * If the XSD copmonent is an attribute and:
     * <ul>
     ...
     *  <li>is local, the value of the "form" attribute determines whether the attribute is qualified
     *      or unqualified.  If the local attribute has no "form" attribute, the value of the schema's 
     *      "attributeFormDefault" attribute determines whether the attribute is qualified or unqualified</li>
     * </ul>
     * </p>
     * <p>
     * Per the XSD specification, the form is determined completely by the element/attribute declaration
     * and it's parent schema.  (This means that element/attribute refs nor their schema are used
     * to determine whether an element/attribute should be qualified.)  Therefore, we always
     * resolve any references.
     * </p>
     * @param component the XML Schema component
     * @return true if entity is to be unqualified.
     */
    protected static boolean isPrefixRequired( final XSDComponent component, final XSDSchema schema,
        ...
        if ( global ) {
            if ( schemaNamespaceUri == null || schemaNamespaceUri.trim().length() == 0 ) {
                // There is no schema namespace, so it is unqualified
                return false;
            }
        ...
        if ( component instanceof XSDFeature ) {    // supertype of both XSDAttributeDeclaration and XSDElementDeclaration
            ...
            if ( feature.isSetForm() ) {
                ...
                // There is no form attribute, so go to the schema and look for the default
                if ( component instanceof XSDElementDeclaration ) {
                    if ( schema.isSetElementFormDefault() ) {
                        form = schema.getElementFormDefault();
                    } else {
                        form = XSDForm.UNQUALIFIED_LITERAL;
                    }
    ...
    /**
     * Method to return the complete value for the "soap-enc:arrayType" attribute on the supplied element.
     * This method checks the schema component referenced by the element, and sees if that complex type
     * has (eventually) a base type of the SOAP-ENC XSD's "ArrayType".
     ...
     */
    protected static String getSoapArrayType( final XmlElement element, final NamespaceContext context, final MappingContext mappingContext ) {
        XSDComponent xsdComponent = element.getXsdComponent();
        while ( xsdComponent != null ) {
            if ( xsdComponent instanceof XSDComplexTypeDefinition ) {
                ...
                if (XSDConstants.isAnyType(complexTypeDefn)){
                    return null;
                }
                ...
                if ( Soap.ARRAY_TYPE_NAME.equals(name) ) {
                    ...
                    if ( Soap.TARGET_NAMESPACE_URI.equals(targetNS) ) {
                        ...
                        String namespacePrefix = getNamespacePrefix(schemaCompOfChildren,context,mappingContext,null,false,true);
                        // If the namespace prefix is null 
                        if ( namespacePrefix == null ) {
                            // See if the schema component is one of the built-ins (in XMLSchema-instance)
                            if ( XsdUtil.isBuiltInDatatype(schemaCompOfChildren) ) {
                                namespacePrefix = getSchemaNamespacePrefix(context);
                            }
                        }

                        String result = null;
                        if ( namespacePrefix != null && namespacePrefix.trim().length() != 0 ) {
                            result = namespacePrefix + ":" + schemaCompOfChildren.getName() + "[]"; //$NON-NLS-1$ //$NON-NLS-2$
                        } else {
                            result = schemaCompOfChildren.getName() + "[]"; //$NON-NLS-1$
                        }
                        return result;
                    }
                } else {
                    // This complex type is not the SOAP ArrayType, so go to the base type ...
                    xsdComponent = complexTypeDefn.getBaseTypeDefinition();
                }
            }
            if ( xsdComponent instanceof XSDSimpleTypeDefinition ) {
                // ArrayType is a complex type, so this is definitely not a SOAP ArrayType
                return null;
            }
            ...
            if ( !(xsdComponent instanceof XSDComplexTypeDefinition) 
              && !(xsdComponent instanceof XSDSimpleTypeDefinition) 
              && !(xsdComponent instanceof XSDElementDeclaration) ) {
                return null;
            }
    ...
    private static String getSchemaInstanceNamespacePrefix( final NamespaceContext context ) {
        ...
        while (iter.hasNext()) {
            ...
            if ( uri != null && uri.startsWith("http://www.w3.org/") && uri.endsWith("/XMLSchema-instance") ) { //$NON-NLS-1$ //$NON-NLS-2$
                return getNamespacePrefix(nsDecl);
            }
        }
        // Add the soap encoding namespace ...
    ...
    public static String getSchemaNamespacePrefix( final NamespaceContext context ) {
        ...
        while (iter.hasNext()) {
            final XmlNamespace nsDecl = (XmlNamespace)iter.next();
            final String uri = nsDecl.getUri();
            if ( uri != null && uri.startsWith("http://www.w3.org/") && uri.endsWith("/XMLSchema") ) { //$NON-NLS-1$ //$NON-NLS-2$
                return getNamespacePrefix(nsDecl);
            }
        }
        // Add the soap encoding namespace ...
