...
public class MappingDocumentFormatter {
    ...
    public static MappingDocumentFormatter create( final XmlDocument xmlDoc,
        ...
        if (treeMappingRoots != null && treeMappingRoots.size() != 0) {
            ...
            if (mappingClassSet != null) {
                final MappingDocumentFormatter formatter = new MappingDocumentFormatter(xmlDoc, treeMappingRoots,
                                                                                        mappingClassSet, mappingContext);
                return formatter;
            }
        }
        return null;
    ...
    String getRecursionCriteria( XmlElement element ) {
        ...
        if (mappingClass != null && mappingClass.isRecursionAllowed() && mappingClass.isRecursive()) {
            ...
            if (!CoreStringUtil.isEmpty(criteria)) {
                criteria = CoreStringUtil.replaceAll(criteria, getFullName(mappingClass), getRecursionMappingClass(element));
            }
        }
        return criteria;
    }
     ...
     * @param rootElement
     */
    protected MappingAttribute createXsiTypeAttribute( MappingElement parent,
                                                       final ElementInfo elementInfo ) {
    ...
    /**
     * Utility to get a non-empty namespace prefix regardless of what the actual XmlNamespace has for it's prefix. This method
     * does not actually change the XmlNamespace object.
     * 
     * @param ns
     * @return
     */
    ...
    /**
     * Return the XSDSchema reference for the specified XSDComponent or null if the XSDComponent is null. If the XSDComponent is
     * an eProxy the method will try to resolve the XSDResource. If the component is a reference the method will return the
     * referenced schema.
     * 
     * @param xsdComponent
     * @param context
    ...
    private static XSDSchema getTargetNamespaceSchema( XSDComponent xsdComponent,
        ...
        if (xsdComponent instanceof XSDFeature) {
            ...
            if (resolvedComponent != null && resolvedComponent != xsdComponent) {
                ...
                if (CoreStringUtil.isEmpty(resolvedTNS)) {
                ...
                if (resolvedTNS != null && resolvedTNS.trim().length() != 0) {
                    ...
                    if (resolvedSchema != null && resolvedSchema.eIsProxy()) {
                        ...
                        if (proxyURI != null) {
                            XSDResourceImpl resource = (XSDResourceImpl)mappingContext.getResourceSet().getResource(proxyURI.trimFragment(),
                                                                                                                    true);
                            if (resource != null) {
    ...
    /**
     * Return the target namespace string for the specified XmlDocumentNode. If the XmlDocumentNode has no XSDComponent reference
     * or is determined to reference a global schema then null is returned.
     * 
     * @param element
     * @param context
     * @param mappingContext
     ...
     * schema component referenced by the element, and sees if that complex type has (eventually) a base type of the SOAP-ENC
     * XSD's "ArrayType".
     * 
