...
public class CompositeMetadataStore extends MetadataStore {
	...
	public Table findGroup(String fullName)	throws Exception {
		int index = fullName.indexOf(TransformationMetadata.DELIMITER_STRING);
		if (index == -1) {
		     throw new TeiidClientException(fullName+TransformationMetadata.NOT_EXISTS_MESSAGE);
		}			
	...
	public Collection<Procedure> getStoredProcedure(String name) throws Exception {
		...
		if (index > -1) {
			...
			Schema schema = getSchema(schemaName);
			if (schema == null ) {
		         throw new TeiidClientException(name+TransformationMetadata.NOT_EXISTS_MESSAGE);			
			}			
