...
public class UpdateProcedureResolver extends CommandResolver {
    ...
    public void resolveCommand(Command command, TempMetadataAdapter metadata, boolean resolveNullLiterals)
        ...
        ProcedureContainerResolver.addScalarGroup(getTeiidParser(), ProcedureReservedWords.VARIABLES, metadata.getMetadataStore(), externalGroups, symbols);    
    	
    	if (command instanceof TriggerAction) {
    		TriggerAction ta = (TriggerAction)command;
    		CreateProcedureCommand cmd = create(ASTNodes.CREATE_PROCEDURE_COMMAND); 
    		...
    		cmd.setUpdateType(ICommand.TYPE_INSERT);
	...
	public void resolveBlock(CreateProcedureCommand command, Block block, GroupContext originalExternalGroups, 
                              TempMetadataAdapter original) throws Exception {
//	      TODO
//        LogManager.logTrace(org.teiid.logging.LogConstants.CTX_QUERY_RESOLVER, new Object[]{"Resolving block", block}); //$NON-NLS-1$
        ...
        GroupSymbol variables = ProcedureContainerResolver.addScalarGroup(getTeiidParser(), ProcedureReservedWords.VARIABLES, store, externalGroups, new LinkedList<Expression>());
        ...
        if (block.getExceptionGroup() != null) {
            ...
            //create a new variables group for this block
            variables = ProcedureContainerResolver.addScalarGroup(getTeiidParser(), ProcedureReservedWords.VARIABLES, store, externalGroups, new LinkedList<Expression>());
            isValidGroup(metadata, block.getExceptionGroup());
            
            if (block.getExceptionStatements() != null) {
            	ProcedureContainerResolver.addScalarGroup(getTeiidParser(), block.getExceptionGroup(), store, externalGroups, exceptionGroup, false);
...
//        LogManager.logTrace(org.teiid.logging.LogConstants.CTX_QUERY_RESOLVER, new Object[]{"Resolving statement", statement}); //$NON-NLS-1$
        ...
        switch(statement.getType()) {
            ...
            case TYPE_LOOP:
                ...
                ProcedureContainerResolver.addScalarGroup(loopStmt.getTeiidParser(), groupName, store, externalGroups, symbols, false);
    ...
    private void collectDeclareVariable(DeclareStatement obj, GroupSymbol variables, TempMetadataAdapter metadata, GroupContext externalGroups) throws Exception {
        ...
        if(gs == null) {
        	...
        	if (gs.getSchema() != null || !gs.getShortName().equalsIgnoreCase(ProcedureReservedWords.VARIABLES)) {
                handleUnresolvableDeclaration(variable, Messages.getString(Messages.ERR.ERR_015_010_0031, new Object[]{ProcedureReservedWords.VARIABLES, variable})); //$NON-NLS-1$
            }
        }
