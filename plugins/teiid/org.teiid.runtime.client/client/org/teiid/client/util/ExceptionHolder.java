...
public class ExceptionHolder implements Externalizable {
	...
	private Throwable buildException(List<String> classNames, String message, StackTraceElement[] stackTrace, String code) {
		...
		List<String> args = Arrays.asList(Messages.getString(Messages.Misc.ExceptionHolder_convertedException, message, originalClass)); //$NON-NLS-1$
		...
		if (result == null) {
			result = new RuntimeException(args.get(0));
		}
