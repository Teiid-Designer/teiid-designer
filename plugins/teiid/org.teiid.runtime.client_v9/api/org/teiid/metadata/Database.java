...
public class Database extends AbstractMetadataRecord {
    ...
    public Database(ITeiidServerVersion teiidVersion, String dbName) {
        super.setName(dbName);
        this.teiidVersion = teiidVersion;
        this.version = "1";
    }
    
    public Database(ITeiidServerVersion teiidVersion, String dbName, String version) {
        super.setName(dbName);
        this.teiidVersion = teiidVersion;
    ...
    public void addSchema(Schema schema) {
        ...
        if ( s != null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60021, schema.getName()));
        } else {
    ...
    public Schema removeSchema(String schemaName) {
        ...
        if (s == null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60024, schemaName));            
        }
    ...
    public void addDataWrapper(DataWrapper wrapper) {
        ...
        if (w != null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60022, wrapper.getName()));
        } else {
    ...
    public DataWrapper removeDataWrapper(String wrapperName) {
        ...
        if (wrapper == null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60023, wrapperName));            
        }
    ...
    public void addServer(Server server) {
        ...
        if (s != null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60026, server.getName()));            
        } else {
    ...
    public Server removeServer(String serverName) {
        ...
        if (server == null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60027, serverName));            
        } 
    ...
    public void addNamespace(String prefix, String uri) {
        ...
        if (StringUtil.startsWithIgnoreCase(prefix, MetadataFactory.TEIID_RESERVED)) {
            String validURI = MetadataFactory.builtinNamespaces(teiidVersion).get(prefix);
    ...
    public String resolveNamespaceInPropertyKey(String key) {
        ...
        if (index > 0 && index < key.length() - 1) {
            ...
            String uri = MetadataFactory.builtinNamespaces(teiidVersion).get(prefix);
            if (uri == null) {
    ...
    public void addRole(Role role) {
        ...
        if ( r != null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60028, role.getName()));
        } else {
    ...
    public void removeRole(String roleName) {
        ...
        if (r == null) {
            throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60029, roleName));            
        } else {
            // make sure it is not used in any grants
            for (Grant g:this.store.getGrants()) {
                if (g.getRole().equalsIgnoreCase(roleName)) {
                    throw new RuntimeException(Messages.gs(Messages.TEIID.TEIID60030, roleName,
