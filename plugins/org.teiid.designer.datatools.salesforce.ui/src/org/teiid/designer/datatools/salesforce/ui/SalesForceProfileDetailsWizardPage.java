 ...
 * 
 */
public class SalesForceProfileDetailsWizardPage extends ConnectionProfileDetailsPage implements Listener {
    ...
    public SalesForceProfileDetailsWizardPage( String pageName ) {
        super(pageName, Messages.getString("SalesForceProfileDetailsWizardPage.Name"), //$NON-NLS-1$
    ...
    public void createCustomControl( Composite parent ) {
        ...
        profileLabel.setText(Messages.getString("Common.Profile.Label")); //$NON-NLS-1$
        gd = new GridData();
        ...
        descriptionLabel.setText(Messages.getString("Common.Description.Label")); //$NON-NLS-1$
        gd = new GridData();
        gd.verticalAlignment = GridData.BEGINNING;
        ...
        urlCheckBox.setText(Messages.getString("Common.URL.Label")); //$NON-NLS-1$
        urlCheckBox.setToolTipText(Messages.getString("Common.URL.CheckBox.ToolTip")); //$NON-NLS-1$
    ...
    void updateState() {
        ...
        if (null == properties.get(ISalesForceProfileConstants.USERNAME_PROP_ID)
            ...
            setErrorMessage(Messages.getString("Common.Username.Error.Message")); //$NON-NLS-1$
            setPingButtonEnabled(false);
            return;
        }
        ...
        if (null == properties.get(ISalesForceProfileConstants.PASSWORD_PROP_ID)
            ...
            setErrorMessage(Messages.getString("Common.Password.Error.Message")); //$NON-NLS-1$
            setPingButtonEnabled(false);
            return;
        }
        setErrorMessage(null);
        if (urlCheckBox.getSelection()) {
            if (null == properties.get(ISalesForceProfileConstants.URL_PROP_ID)
                ...
                setErrorMessage(Messages.getString("Common.URL.Error.Message")); //$NON-NLS-1$
                setPingButtonEnabled(false);
                return;
            }
        } else {
        ...
        if (!isValidatedConnection()) {
            setMessage(Messages.getString("Click.Test.Connection")); //$NON-NLS-1$
            setPingButtonEnabled(true);
        } else {
            setPageComplete(true);
            setMessage(Messages.getString("Click.Next")); //$NON-NLS-1$
        }
    ...
    public List getSummaryData() {
        ...
        result.add(new String[] {Messages.getString("Common.Username.Label"), usernameText.getText()}); //$NON-NLS-1$
        if (urlCheckBox.getSelection()) {
            result.add(new String[] {Messages.getString("Common.URL.Label"), urlText.getText()}); //$NON-NLS-1$
        }
        return result;
    }
}
