...
public class ModelExtensionDefinitionParser {
    ...
    /**
     * Get the Model Extension Definition template, based on the modelExtension.xsd
     * 
     * @return empty template model extension definition (never <code>null</code>)
     * @throws Exception if the definition file is <code>null</code> or if there is a problem parsing the file
     */
    public InputStream getModelExtensionDefinitionTemplate() throws IllegalStateException {
        try {
            // Get the modelExtension schema
            final String TEMPLATE_FILE = "medTemplate.mxd"; //$NON-NLS-1$
            Bundle bundle = Platform.getBundle(PLUGIN_ID);
            URL url = bundle.getEntry(TEMPLATE_FILE);

            if (url == null) {
                throw new IllegalStateException(NLS.bind(Messages.medTemplateFileNotFoundInWorkspace, TEMPLATE_FILE));
            }

            File definitionSchemaFile = new File(FileLocator.toFileURL(url).getFile());

            if (!definitionSchemaFile.exists()) {
                throw new IllegalStateException(NLS.bind(Messages.medTemplateFileNotFoundInFilesystem, TEMPLATE_FILE));
            }

            return new FileInputStream(definitionSchemaFile);
        } catch (Exception e) {
            IllegalStateException error = null;
     ...
     */
    // public InputStream getModelExtensionDefinitionTemplate() throws IllegalStateException {
    // File definitionSchemaFile = null;
    // try {
    // // Get the modelExtension schema
    //            final String SCHEMA_FILE = "modelExtension.xsd"; //$NON-NLS-1$
    // Bundle bundle = Platform.getBundle(PLUGIN_ID);
    ...
    // }
    //
    // definitionSchemaFile = new File(FileLocator.toFileURL(url).getFile());
    //
    // if (!definitionSchemaFile.exists()) {
    // throw new IllegalStateException(NLS.bind(Messages.definitionSchemaFileNotFoundInFilesystem, SCHEMA_FILE));
    // }
    ...
    //
    // try {
    // // Create a temp file for the empty mxd
    //            tempFile = File.createTempFile("MxdTemp", ".mxd"); //$NON-NLS-1$ //$NON-NLS-2$
    //
    ...
    //            Element modelExtension = document.createElement("modelExtension"); //$NON-NLS-1$
    //
    // // Child - description element
    //            Element description = document.createElement("description"); //$NON-NLS-1$
    // Text descriptionText = document.createTextNode("[Description Goes Here]");
    // description.appendChild(descriptionText);
    //
    // // Child - extended metamodel class element
    // Element extendedMetaclass = document.createElement("extendedMetaclass");
    // Attr attr = document.createAttribute("name");
    // attr.setValue("[extended metaclass name]");
    // extendedMetaclass.setAttributeNode(attr);
    //
    // Element crap = document.createElement("crap");
    //
    // // Append Child Nodes to modelExtension element
    // document.appendChild(modelExtension);
    // modelExtension.appendChild(description);
    // modelExtension.appendChild(extendedMetaclass);
    // modelExtension.appendChild(crap);
    //
    // // Output it
