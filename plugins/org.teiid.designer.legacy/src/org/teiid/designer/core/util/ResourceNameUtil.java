...
public abstract class ResourceNameUtil {
    ...
    public static final String[] RESERVED_XMI_NAMES = {CORE_NAME, BUILTINRELATIONALTYPES_NAME, DATAACCESS_NAME,
        DATASERVICESYSTEMMODEL_NAME, DTCBASE_NAME, ECORE_NAME, ENTERPRISEDATATYPES_NAME, EXTENSION_NAME, FUNCTION_NAME, JDBC_NAME,
        JDBCMODEL_NAME, JDBCSYSTEM_NAME, MANIFEST_NAME, MAPPING_NAME, MBR_NAME, METAMODELRELATIONALMODEL_NAME, PRIMATIVETYPES_NAME,
        RELATIONAL_NAME, RELATIONSHIP_NAME, SIMPLEDATATYPES_INSTANCE_NAME, SYSTEM_NAME, TRANSFORMATION_NAME, UML2_NAME,
    ...
    public static boolean isReservedModelName( String proposedName ) throws IllegalArgumentException {
        ...
        if (proposedName.indexOf('.') != -1) {
            // Check ends with
            if (!proposedName.endsWith(DOT_XMI_FILE_EXTENSION)) throw new IllegalArgumentException(
                                                                                                   CoreModelerPlugin.Util.getString(INVALID_EXTENSION_ERROR_ID,
                                                                                                                             proposedName,
                                                                                                                             XMI_FILE_EXTENSION));
            ...
            // Note
            if (proposedName.indexOf('.') != -1) return false;
        }

        for (final String element : RESERVED_XMI_NAMES)
    ...
    public static boolean isReservedSchemaName( String proposedName ) throws IllegalArgumentException {
        ...
        if (proposedName.indexOf('.') != -1) {
            // Check ends with
            if (!proposedName.endsWith(DOT_XSD_FILE_EXTENSION)) throw new IllegalArgumentException(
                                                                                                   CoreModelerPlugin.Util.getString(INVALID_EXTENSION_ERROR_ID,
                                                                                                                             proposedName,
                                                                                                                             XSD_FILE_EXTENSION));
        ...
        for (final String element : RESERVED_XSD_NAMES)
            if (proposedName.equalsIgnoreCase(element)) return true;
    ...
    /**
     * This method checks whether or not a proposed vdb name is reserved or not. It will return false if the proposed name includes
     * an extension AND one or more "." characters.
     * 
     * @param proposedName may or may not inlude the file extension
     * @return true if it is reserved, false if not.
     * @throws IllegalArgumentException if proposed name contains an apparent file extension (one or more '.' characters) and it is
    ...
    public static boolean isReservedVdbName( String proposedName ) throws IllegalArgumentException {
        ...
        if (proposedName.indexOf('.') != -1) {
            // Check ends with
            if (!proposedName.endsWith(DOT_VDB_FILE_EXTENSION)) throw new IllegalArgumentException(
                                                                                                   CoreModelerPlugin.Util.getString(INVALID_EXTENSION_ERROR_ID,
                                                                                                                             proposedName,
                                                                                                                             VDB_FILE_EXTENSION));
        ...
        for (final String element : RESERVED_VDB_NAMES)
            if (proposedName.equalsIgnoreCase(element)) return true;
