	...
	private void createPanel(Composite parent) {
		...
		autoGenRESTCheckbox.addSelectionListener(new SelectionListener() {
            ...
            public void widgetDefaultSelected( SelectionEvent e ) {
            	...
            	try {
        			validRestVdb = false; // TODO: !!!!! fix this???   RestVdbUtil.isRestWarVdb(vdb.getFile());
        		} catch (Exception ex) {
...
//    		gd_2.horizontalSpan = 2;
            ...
            for( String value : vdb.getAllowedLanguages().getAllowedLanguageValues() ) {
    ...
    void handleAddLanguage() {
        ...
        AddLanguagePropertyDialog dialog = 
        		new AddLanguagePropertyDialog(allowedLanguagesViewer.getControl().getShell(), currentValues);

        if (dialog.open() == Window.OK) {
            // update model
            String language = dialog.getLanguage();

            vdb.getAllowedLanguages().addAllowedLanguage(language);
    ...
    void handleRemoveLanguage() {
        ...
        // update model
        this.vdb.getAllowedLanguages().removeAllowedLanguage(selectedLanguage);
