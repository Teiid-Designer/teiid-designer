		...
		SOURCE_BINDING_PANEL : {
			...
			multiSourceCB.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					if( model != null ) {
						model.setAllowMultiSource(multiSourceCB.getSelection());
					}
					refreshModelDetails();
				}
			...
			addColumnCB.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					if( model != null ) {
						model.setAddColumn(addColumnCB.getSelection());
					}
					refreshModelDetails();
				}
			...
			columnAliasText.addModifyListener(new ModifyListener() {
				...
				public void modifyText(ModifyEvent e) {
					model.setColumnAlias(columnAliasText.getText());
				}
			});
			...
			addButton.addSelectionListener(new SelectionAdapter() {
				...
				@Override
				public void widgetSelected(SelectionEvent e) {
					int numSources = model.getSources().length;
					boolean added = false;
					while(!added) {
			...
			BINDING_TABLE : {
		        ...
		        column.setEditingSupport(new TranslatorEditingSupport(bindingsViewer.getTableViewer()));
		        ...
		        column.setEditingSupport(new JndiEditingSupport(bindingsViewer.getTableViewer()));
		...
		NAME_LOCATION_DESCRIPTION_PANEL : {
	        ...
	        this.modelDescriptionEditor.getDocument().addDocumentListener(new IDocumentListener() {
	            ...
	            public void documentChanged( final DocumentEvent event ) {
	            	if( model != null ) {
	            		model.setDescription(modelDescriptionEditor.getText());
	            	}
	            }
...
//        	problemsViewer.getTable().removeAll();
//        	for( Problem prob : model.getProblems() ) {
//        		problemsViewer.add(prob);
//        	}
        	
        	boolean enable = model.isMultiSource();
    		...
    		deleteButton.setEnabled(model.getSources().length > 1 && !bindingsViewer.getSelection().isEmpty());
