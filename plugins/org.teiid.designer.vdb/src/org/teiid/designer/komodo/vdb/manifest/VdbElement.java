...
public class VdbElement implements Serializable {
    ...
    public VdbElement( final Vdb vdb ) throws Exception {
        ...
        for (final Model model : vdb.getModels())
            getModels().add(new ModelElement(model));
            
            for (final VdbImport importVdb : vdb.getImports())
        	getImportVdbEntries().add(new ImportVdbElement(importVdb));
        
        for (final Translator translator : vdb.getTranslators())
            getTranslators().add(new TranslatorElement(translator));
        
        for (final DataRole dataRole : vdb.getDataRoles())
            getDataPolicies().add(new DataRoleElement(dataRole));
    	...
    	String valueStr = vdb.getAllowedLanguages().getOutputString();
