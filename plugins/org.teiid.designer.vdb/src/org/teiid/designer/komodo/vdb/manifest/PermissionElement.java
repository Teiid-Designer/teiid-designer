...
public class PermissionElement implements Serializable {
    ...
    public PermissionElement(Permission permission) {
        super();
        this.resource_name = permission.getName();
        ...
        if( !permission.isAllowLanguage() ) {
	        ...
	        Condition cond = permission.getCondition();
	        if( cond != null && cond.getName() != null && cond.getName().length() > 0 ) {
	        	condition = new ConditionElement(cond.getName(), cond.isConstraint());
	        }
	        
	        Mask theMask = permission.getMask();
	        if( theMask != null && theMask.getName() != null && theMask.getName().length() > 0 ) {
	        	mask = new MaskElement(theMask.getName(), theMask.getOrder());
	        }
