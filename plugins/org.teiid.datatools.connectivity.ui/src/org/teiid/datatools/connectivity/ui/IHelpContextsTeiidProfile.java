/*
 * JBoss, Home of Professional Open Source.
 *
 * See the LEGAL.txt file distributed with this work for information regarding copyright ownership and licensing.
 *
 * See the AUTHORS.txt file distributed with this work for a full listing of individual contributors.
 */
package org.teiid.datatools.connectivity.ui;

/**
 * @since 8.0
 */
public interface IHelpContextsTeiidProfile {

    public static final String TEIID_PROFILE_WIZARD_PAGE = "TEIID_PROFILE_WIZARD_PAGE"; //$NON-NLS-1$

    public static final String TEIID_PROFILE_PROPERTY_PAGE = "TEIID_PROFILE_PROPERTY_PAGE"; //$NON-NLS-1$

    public static final String TEIID_PROFILE_WIZARD = "TEIID_PROFILE_WIZARD"; //$NON-NLS-1$
}
