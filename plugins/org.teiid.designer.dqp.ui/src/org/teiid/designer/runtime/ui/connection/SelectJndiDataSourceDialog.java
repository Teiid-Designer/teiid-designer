...
public class SelectJndiDataSourceDialog  extends ElementTreeSelectionDialog implements ISelectionChangedListener {
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if(!(firstElement instanceof TeiidDataSource) ) {
			...
			this.dataSourceNameText.setText(selectedDataSource.getDisplayName());
		}
	...
	private void updateOnSelection(Object selectedObject) {
		IStatus status = new Status(IStatus.INFO, DqpUiPlugin.PLUGIN_ID, DqpUiPlugin.UTIL.getString("SelectJndiDataSourceDialog.okSelectionMessage")); //$NON-NLS-1$
		if( selectedObject != null ) {
			if( !(selectedObject instanceof TeiidDataSource) ) {
				status = new Status(IStatus.ERROR, DqpUiPlugin.PLUGIN_ID, DqpUiPlugin.UTIL.getString("SelectJndiDataSourceDialog.invalidSelectionMessage")); //$NON-NLS-1$
