/*
 * JBoss, Home of Professional Open Source.
*
* See the LEGAL.txt file distributed with this work for information regarding copyright ownership and licensing.
*
* See the AUTHORS.txt file distributed with this work for a full listing of individual contributors.
*/
package org.teiid.designer.runtime.ui.vdb;

/**
 * @since 8.0
 */
public interface VdbConstants {
	final String VDB_EXTENSION = "vdb"; //$NON-NLS-1$
    final String DEFAULT_USERNAME = "admin"; //$NON-NLS-1$
    final String DEFAULT_PASSWORD = "teiid"; //$NON-NLS-1$

    final String DTP_PERSPECTIVE = "org.eclipse.datatools.sqltools.sqleditor.perspectives.EditorPerspective"; //$NON-NLS-1$
}
