...
public class ExecuteVDBAction extends Action
	...
	protected static final String I18N_PREFIX = I18nUtil
			.getPropertyPrefix(ExecuteVdbAction.class);
	...
	public ExecuteVDBAction() {
		super();
		setImageDescriptor(DqpUiPlugin.getDefault().getImageDescriptor(
				DqpUiConstants.Images.DEPLOY_VDB));
	}
		...
		if (!SelectionUtilities.isMultiSelection(selection)) {
			...
			if (obj instanceof IFile) {
				...
				if (extension != null && extension.equals("vdb")) { //$NON-NLS-1$
					...
					if (server != null) {
						try {
							if (null != server.getAdmin().getVdb(
									selectedVDB.getName())) {
								result = true;
							}
						} catch (Exception e) {
