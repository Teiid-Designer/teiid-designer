...
public class DefaultWebArchiveBuilderImpl implements WebArchiveBuilder {
	...
	/**
	 * This constructor is package protected, so that only the factory can call
	 * it.
	 * 
	 * @since 7.1
	 */
	...
	/**
	 * @param ports
	 *            Sets ports to the specified value. These names are used for
	 *            the dynamic generation of servlets in the web.xml, the
	 *            jbossws-cxf.xml endpoints and the wsprovider java files.
	 */
	...
	/**
	 * @param operationToProcedureMap
	 *            Sets operationToProcedureMap to the specified value.
	 */
	public void setOperationToProcedureMap(
			Map<String, String> operationToProcedureMap) {
	...
	public IStatus validateContextName(String contextName) {
		...
		try {
			...
			final String invalidChar = validateInvalidCharactersInContextName(
					contextName, invalidChars);
			...
			if (invalidChar != null) {
				...
				if (StringUtil.Constants.SPACE.equals(invalidChar)) {
					...
					msg = getString(
							"ContextNameValidationFailed_InvalidCharacter", params); //$NON-NLS-1$
				}

				return new Status(
						IStatus.ERROR,
						WebServicePlugin.PLUGIN_ID,
						WebArchiveBuilderConstants.STATUS_CODE_CONTEXT_NAME_VALIDATION_FAILED,
						msg, null);
				...
				return new Status(
						IStatus.OK,
						WebServicePlugin.PLUGIN_ID,
						WebArchiveBuilderConstants.STATUS_CODE_CONTEXT_NAME_VALIDATION_SUCCEEDED,
						msg, null);
			}
		} catch (Exception e) {
	...
	/**
	 * @see com.metamatrix.modeler.dataservices.lds.WebArchiveBuilder#createWebArchive(java.io.InputStream,
	 *      java.util.Map)
	 * @since 7.1
	 */
	public IStatus createWebArchive(Properties properties,
		...
		try {
			...
			final String contextName = (String) properties
					.get(WebArchiveBuilderConstants.PROPERTY_CONTEXT_NAME);
			...
			// exist.
			final String webServicePluginPath = getWSPluginInstallPath()
					.toOSString();
			final String buildDirectoryName = webServicePluginPath
					+ File.separator + WebArchiveBuilderConstants.BUILD_DIR;
			...
			// try to empty it.
			final String workDirectoryName = buildDirectoryName
					+ File.separator + WebArchiveBuilderConstants.WORK_DIR;
			File workDirectory = new File(workDirectoryName);
			workDirectory.mkdir();
			FileUtils.removeChildrenRecursively(workDirectory);
			// Create the temporary directory.
			final TempDirectory tempDir = TempDirectory
					.getTempDirectory(workDirectoryName);
			final String tempDirectoryName = tempDir.getPath();
			// Create context directory.
			final String contextDirectoryName = tempDirectoryName
					+ File.separator + contextName;
			final File contextDirectory = new File(contextDirectoryName);
			contextDirectory.mkdir();
			// Create the images directory.
			final String imagesDirectoryName = contextDirectoryName
					+ File.separator + "images"; //$NON-NLS-1$
			final File imagesDirectory = new File(imagesDirectoryName);
			imagesDirectory.mkdir();
			// Create the WEB-INF directory.
			final String webInfDirectoryName = contextDirectoryName
					+ File.separator + "WEB-INF"; //$NON-NLS-1$
			final File webInfDirectory = new File(webInfDirectoryName);
			webInfDirectory.mkdir();
			// Create the classes directory.
			final String webInfClassesDirectoryName = webInfDirectoryName
					+ File.separator + "classes"; //$NON-NLS-1$
			// Create the classes directory.
			final String webInfLibDirectoryName = webInfDirectoryName
					+ File.separator + "lib"; //$NON-NLS-1$
			final File webInfClassesDirectory = new File(
					webInfClassesDirectoryName);
			...
			createProviderJavaClasses(webInfLibDirectory,
					webInfClassesDirectory, properties);

			monitor.worked(10);

			monitor.subTask(TASK_CREATING_WAR_ARCHIVE);
			// ZIP everything in the context directory into the new WAR file.
			final String warFileName = tempDirectoryName + File.separator
			...
			final String newWarFileDir = (String) properties
					.get(WebArchiveBuilderConstants.PROPERTY_WAR_FILE_SAVE_LOCATION);
			...
			final String newWarFileName = getFileName(
					(String) properties
							.get(WebArchiveBuilderConstants.PROPERTY_WAR_FILE_SAVE_LOCATION),
					contextName + ".war"); //$NON-NLS-1$
			...
			final IStatus status = new Status(
					IStatus.ERROR,
					WebServicePlugin.PLUGIN_ID,
					WebArchiveBuilderConstants.STATUS_CODE_WAR_FILE_CREATION_FAILED,
					msg, e);
		...
		final IStatus status = new Status(
				IStatus.OK,
				WebServicePlugin.PLUGIN_ID,
				WebArchiveBuilderConstants.STATUS_CODE_WAR_FILE_CREATION_SUCCEEDED,
				msg, null);
	...
	public boolean targetWarFileExists(Properties properties) {
		...
		// Get and validate the context name.
		final String contextName = properties
				.getProperty(WebArchiveBuilderConstants.PROPERTY_CONTEXT_NAME);
		IStatus status = validateContextName(contextName);
	...
	protected static String getString(final String id, final Object[] params) {
		return WebServicePlugin.Util.getString(I18N_PREFIX + id, params);
	}

	/**
	 * This method validates if any of the specified invalid characters are in
	 * the input context then a null value will be returned. Note: the
	 * characters are in String format.
	 * 
	 * @param contextName
	 * @param chars
	 * @return
	...
	private void getWebFiles(File contextDirectory, File webInfDirectory)
		...
		final String webAppsDirectoryName = webLibPath + File.separator
				+ "webapps"; //$NON-NLS-1$
		final File webAppsDirectory = new File(webAppsDirectoryName);
		FileUtils.copyRecursively(webAppsDirectory, contextDirectory, null,
				false);
	}
	...
	protected void replaceWebXmlVariables(String webInfDirectoryName,
			Properties properties, String contextName) {
		...
		if (properties.getProperty(
				WebArchiveBuilderConstants.PROPERTY_SECURITY_TYPE).equals(
				WarDeploymentInfoPanel.BASIC)) {
			String realm = properties
					.getProperty(WebArchiveBuilderConstants.PROPERTY_SECURITY_REALM);
			String role = properties
	...
	/**
	 * Replace the variables in the jbossws-cxf.xml file with their appropriate
	 * values.
	 * 
	 * @param webInfDirectoryName
	 * @param properties
	...
	protected void replaceJBossWSCXFVariables(String webInfDirectoryName,
		...
		File jbossWSCxfXMLFile = new File(webInfDirectoryName + File.separator
				+ "jbossws-cxf.xml"); //$NON-NLS-1$
	...
	protected void createPropertiesFile(File webInfClassesDirectory,
		...
		File teiisSoapProperties = new File(webInfClassesDirectory
				+ File.separator + "teiidsoap.properties"); //$NON-NLS-1$
		String jndiValue = properties
				.getProperty(WebArchiveBuilderConstants.PROPERTY_JNDI_NAME);
		...
		try {
			...
			out.write(WebArchiveBuilderConstants.PROPERTY_JNDI_NAME
					+ "=" + jndiValue); //$NON-NLS-1$
		} finally {
	...
	protected void createProviderJavaClasses(File webInfLibDirectory,
			File webInfClassesDirectory, Properties properties)
			throws IOException {
		...
		final String tns = (String) properties
				.get(WebArchiveBuilderConstants.PROPERTY_WSDL_TNS);
		String vdbFileName = properties
				.getProperty(WebArchiveBuilderConstants.PROPERTY_VDB_FILE_NAME);
		File vdbFile = new File(FileUtils
				.getFilenameWithoutExtension(vdbFileName));

		List<File> portProviders = new ArrayList<File>();
		for (String port : getPorts()) {
			String providerJavaFilePath = webInfClassesDirectory
					.getCanonicalPath()
					+ pathToProviders + File.separator + port + ".java"; //$NON-NLS-1$
			FileUtils.copy(webInfClassesDirectory.getCanonicalPath()
					+ pathToProviders + File.separator
					+ "ProviderTemplate.java", //$NON-NLS-1$
					providerJavaFilePath, true);
			...
			AntTasks.replace(providerJavaFile,
					"${serviceName}", vdbFile.getName()); //$NON-NLS-1$
			AntTasks.replace(providerJavaFile,
					"${wsdlFileName}", this.wsdlFilename); //$NON-NLS-1$
		}

		File template = new File(webInfClassesDirectory.getCanonicalPath()
				+ pathToProviders + File.separator + "ProviderTemplate.java"); //$NON-NLS-1$

		File soapPlugin = new File(webInfClassesDirectory.getCanonicalPath()
				+ pathToPlugin + File.separator + "SoapPlugin.java"); //$NON-NLS-1$

		File teiidProvider = new File(webInfClassesDirectory.getCanonicalPath()
				+ pathToProviders + File.separator + "TeiidWSProvider.java"); //$NON-NLS-1$

		File usernameCallback = new File(webInfClassesDirectory
				.getCanonicalPath()
				+ pathToCallback
				+ File.separator
	...
	private boolean isWSSecurity(Properties properties) {
		return properties
				.get(WebArchiveBuilderConstants.PROPERTY_SECURITY_TYPE).equals(
						WarDeploymentInfoPanel.WSSE);
	}
	...
	private String getWebLibDirectoryPath() throws Exception {
		final String webServicePluginPath = getWSPluginInstallPath()
				.toOSString();
		final String webServiceLibFolder = webServicePluginPath
				+ File.separator + "war_resources"; //$NON-NLS-1$
		if (new File(webServiceLibFolder).exists()) {
			return webServiceLibFolder;
		}
		final String msg = WebServicePlugin.Util
				.getString(
						"DefaultWebArchiveBuilderImpl.web_lib_directory_does_not_exist", webServiceLibFolder); //$NON-NLS-1$
	...
	public IPath getWSPluginInstallPath() throws IOException {
		if (this.webServicePluginPath == null) {
			URL url = FileLocator.find(WebServicePlugin.getInstance()
					.getBundle(), new Path(""), null); //$NON-NLS-1$
	...
	public void generateWsdl(Properties properties, File classesFolder)
			throws IOException {
		...
		final String contextName = (String) properties
				.get(WebArchiveBuilderConstants.PROPERTY_CONTEXT_NAME);
		final String host = (String) properties
				.get(WebArchiveBuilderConstants.PROPERTY_WAR_HOST);
		final String port = (String) properties
				.get(WebArchiveBuilderConstants.PROPERTY_WAR_PORT);
		final String tns = (String) properties
				.get(WebArchiveBuilderConstants.PROPERTY_WSDL_TNS);
		final ArrayList<ModelResource> modelsArrayList = (ArrayList<ModelResource>) properties
				.get(WebArchiveBuilderConstants.PROPERTY_VDB_WS_MODELS);
		String webServiceName = contextName;
		for (ModelResource webServiceModel : modelsArrayList) {
			try {
				wsModel = webServiceModel;
				wsdlGenerator.addWebServiceModel(webServiceModel
						.getEmfResource());
				...
				for (IResource iResource : dependentSchemas) {
					if (ModelIdentifier.isSchemaModel(iResource)) {
						...
						if (containsGlobalDataTypeImport(xsdSchema)) {
							...
							Resource builtInDataypesResource = ModelerSdtPlugin
									.getBuiltInTypesResource();
							FileUtils.copy(new File(builtInDataypesResource
									.getURI().path()), classesFolder, true);
							...
							FileUtils.copy(new File(iResource.getLocation()
		...
		wsdlGenerator
				.setUrlRootForReferences(StringUtil.Constants.EMPTY_STRING);
		wsdlGenerator
				.setUrlSuffixForReferences(StringUtil.Constants.EMPTY_STRING);
		wsdlGenerator
				.setUrlForWsdlService("http://" + host + ":" + port + "/" + contextName + "/"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		final IStatus status = wsdlGenerator
		...
		try {
			...
			OutputStream stream = new FileOutputStream(new File(classesFolder,
					fileName));
			wsdlGenerator.write(stream);
			// Get an iFile instance to refresh our workspace
			IFile iFile = wsModel.getModelProject().getProject().getFile(
					fileName);
			iFile.refreshLocal(IResource.DEPTH_INFINITE,
					new NullProgressMonitor());
			setPorts(wsdlGenerator.getPorts());
			setOperationToProcedureMap(wsdlGenerator
					.getOperationToProcedureMap());
		...
		if (schema != null) {
			...
			for (int i = 0; i < contents.size(); i++) {
				...
				if (content instanceof XSDImport) {
					if (DatatypeConstants.BUILTIN_DATATYPES_URI
							.equals(((XSDImport) content).getNamespace())) {
						containsImport = true;
						break;
					}
				}
	...
	private void getAllDependentSchemas(IResource[] iResources,
		...
		// resources, if any
		for (IResource iResource : iResources) {
			IResource[] moreIResources = WorkspaceResourceFinderUtil
					.getDependentResources(iResource);
	...
	private String createServletTags(List<String> tags) {
		...
		for (String port : tags) {
			servletTags.append(startServlet).append(startServletName).append(
					port).append(endServletName);
			servletTags.append(startServletClass).append(port).append(
					endServletClass).append(endServlet);
			servletTags.append(startServletMapping).append(startServletName)
					.append(port).append(endServletName);
			servletTags.append(startUrlPattern).append(port).append(
					endUrlPattern).append(endServletMapping);
		}

		return servletTags.toString();
	}

	private String createEndpointTags(Properties properties, List<String> tags) {
