...
public class PreviewTableDataContextAction extends SortableSelectionAction  implements IPasswordProvider {
    ...
    @Override
    public void run() {
		if ((Job.getJobManager().find(
				WorkspacePreviewVdbJob.WORKSPACE_PREVIEW_FAMILY).length != 0)
				|| (Job.getJobManager().find(
						TeiidPreviewVdbJob.TEIID_PREVIEW_FAMILY).length != 0)) {
			PreviewUnavailableDialog dialog = new PreviewUnavailableDialog(
					getShell());
			dialog.open();
	...
	class PreviewUnavailableDialog extends MessageDialog {
		...
		public PreviewUnavailableDialog(Shell parent) {
			super(
					parent,
					DqpUiConstants.UTIL
							.getString("PreviewTableDataContextAction.previewUnavailableDialog.title"), null, //$NON-NLS-1$
					DqpUiConstants.UTIL
							.getString("PreviewTableDataContextAction.previewUnavailableDialog.message"), //$NON-NLS-1$
		...
		protected Control createCustomArea(Composite parent) {
			...
			btnOpenProgressView
					.setText(DqpUiConstants.UTIL
							.getString("PreviewTableDataContextAction.previewUnavailableDialog.btnShowProgressView.text")); //$NON-NLS-1$
			btnOpenProgressView
					.setToolTipText(DqpUiConstants.UTIL
							.getString("PreviewTableDataContextAction.previewUnavailableDialog.btnShowProgressView.toolTip")); //$NON-NLS-1$
