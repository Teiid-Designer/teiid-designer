...
import org.teiid.designer.wsdl.soap.SOAPSchemaProcessor;
import org.teiid.designer.wsdl.soap.SoapBindingInfo;
import org.teiid.designer.wsdl.soap.SOAPTable;
import org.teiid.designer.wsdl.util.NameUtil;
...
public class TableBuilder {
    ...
    public Collection createTables( Operation[] opers ) throws ModelBuildingException {
        ...
        try {
            ...
            for (Iterator messageIter = messages.iterator(); messageIter.hasNext();) {
                ...
                SOAPTable baseTableWrapper = new SOAPTable(baseTable, message.isRequest(), soapAction, info);
                ...
                for (int i = 0; i < parts.length; i++) {
                    ...
                    if (elementNamespace.equalsIgnoreCase(XML_SCHEMA_URI)) {
                        ...
                        // get the root table
                        SOAPTable top = new SOAPTable(getRootTable(tabList, topName));
    ...
    private void processChildren( Table[] tabs,
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            tables.put(tableName, new SOAPTable(table, isRequest, null, null));
    ...
    private void processChildren( Table table,
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            mergeNamespacePrefixes(table, baseTable);
            tables.put(tableName, new SOAPTable(tabs[i], isRequest, null, null));
    ...
    private void checkForForeignKeys( Table parent,
        ...
        switch (child.getRelationToParent()) {
            ...
            case Relationship.KEY_IN_PARENT_SINGLE:
                ...
                parentKeyColumn.setRole(DATA_ROLE);
            ...
            case Relationship.RELATIONSHIP_TABLE:
                ...
                relParentKeyColumn.setRole(DATA_ROLE);
                ...
                relChildKeyColumn.setRole(DATA_ROLE);
                ...
                tables.put(relTable.getName(), new SOAPTable(relTable, false, null, null));
