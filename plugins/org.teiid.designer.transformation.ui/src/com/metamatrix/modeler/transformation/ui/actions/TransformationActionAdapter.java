...
public class TransformationActionAdapter
  ...
  implements UiConstants, 
    ...
    /**
     * private method used to update all transformation actions so they maintain the
     * reference to the transformation displayed in the transformation diagram.
     *
     */
    ...
    public void contributeToMenuManager(IMenuManager theMenuMgr, ISelection selection) {
        ...
        if( !isDependencyDiagram() ) {
            ...
            TransformationSelectionHelper selectionHelper = 
                new TransformationSelectionHelper(transformationEObject, selection);
    ...
    private void addExtendedActions(IMenuManager theMenuMgr) {
        ...
        if( selProvider != null ) {
            ...
            ControlledPopupMenuExtender popupMenuExtender = 
                new ControlledPopupMenuExtender(ContextMenu.DIAGRAM_EDITOR_PAGE, (MenuManager)theMenuMgr, selProvider, editor );
    ...
    private void addExternalExportedActions(IMenuManager theMenuMgr, ISelection selection) {
        ...
        for (int size = contributors.size(), i = 0; i < size; i++) {
            IModelObjectActionContributor contributor =
                (IModelObjectActionContributor)contributors.get(i);
    ...
    private void removeDiagramActions(IMenuManager theMenuMgr) {
        ...
        if ( theMenuMgr.find( ContextMenu.TRANS_END ) != null )
            theMenuMgr.remove( ContextMenu.TRANS_END );
    }
    ...
    private boolean isDependencyDiagram() {
        ...
        if( editor.getDiagram()!= null && 
            editor.getDiagram().getType()!= null &&
            editor.getDiagram().getType().equals(PluginConstants.DEPENDENCY_DIAGRAM_TYPE_ID))
            return true;
    ...
    private MenuManager getCopyNameSubMenu( ) {   
        ...
        MenuManager menu = new MenuManager( com.metamatrix.modeler.ui.UiConstants.Util.getString( "ModelerActionService.copyNameSubMenu.title" ) );    //$NON-NLS-1$
     ...
     * This was copied from ModelerActionService and changed to only process the trasformation.ui contributor to
     * find the showDependencyAction.
                        ...
                        for (int j = 0; j < elements.length; j++) {
                            try {
                                ...
                                if (extension instanceof IModelObjectActionContributor) {
                                    ...
                                    com.metamatrix.modeler.ui.UiConstants.Util.log(IStatus.ERROR, com.metamatrix.modeler.ui.UiConstants.Util.getString("ModelerActionService.wrongContributorClass", //$NON-NLS-1$
                                    new Object[] { extension.getClass().getName()}));
                                }
                            } catch (Exception theException) {
                                com.metamatrix.modeler.ui.UiConstants.Util.log(IStatus.ERROR, theException, com.metamatrix.modeler.ui.UiConstants.Util.getString("ModelerActionService.contributorProblem", //$NON-NLS-1$
