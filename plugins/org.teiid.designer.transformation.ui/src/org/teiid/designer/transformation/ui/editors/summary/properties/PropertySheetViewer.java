 ...
 * editor
 * 
 * see:  org.eclipse.ui.views.properties.PropertySheetViewer.java
 */
...
public class PropertySheetViewer extends Viewer {
	...
	private ListenerList<ICellEditorActivationListener> activationListeners = new ListenerList<ICellEditorActivationListener>();
	...
	public PropertySheetViewer(Composite parent) {
		...
		GridLayoutFactory.fillDefaults().margins(5, 5).applyTo(tree);
		GridDataFactory.fillDefaults().grab(true,  true).hint(200, 200).applyTo(tree);
	...
	public void setColumnWidths() {
		...
		if (area.width > 0) {
			int minWidth = 120;
			int colOneWidth = area.width * 40 / 100;
			if( colOneWidth < minWidth ) colOneWidth = minWidth;
			
			columns[0].setWidth(colOneWidth);
			columns[1].setWidth(area.width - columns[0].getWidth() - 4);
		}
	}
	...
	private List<Object> getChildren(Object node) {
		...
		if (category == null) {
			List<Object> entries = getChildren(entry);
			children.addAll(entries);
		} else {
			List<Object> categories = getChildren(category);
	...
	private List<Object> getChildren(IPropertySheetEntry entry) {
		...
		if (entry == rootEntry && isShowingCategories) {
			if (categories.length > 1
				...
				for( PropertySheetCategory cat : categories ) {
					entries.add(cat);
				}
				return entries;
			}
		...
		List<IPropertySheetEntry> filterdEntries = getSortedEntries(getFilteredEntries(entry.getChildEntries()));
		List<Object> tmpEntries = new ArrayList<Object>();
		for( IPropertySheetEntry obj : filterdEntries ) {
			tmpEntries.add(obj);
		}
	...
	private List<Object> getChildren(PropertySheetCategory category) {
		List<IPropertySheetEntry> filterdEntries = getSortedEntries(getFilteredEntries(category.getChildEntries()));
		List<Object> tmpEntries = new ArrayList<Object>();
		for( IPropertySheetEntry obj : filterdEntries ) {
			tmpEntries.add(obj);
		}
	...
	private List<IPropertySheetEntry> getFilteredEntries(IPropertySheetEntry[] entries) {
		...
		if (isShowingExpertProperties) {
			List<Object> tmpEntries = new ArrayList<Object>();
			for( IPropertySheetEntry obj : entries ) {
				tmpEntries.add(obj);
			}
		...
		List<Object> tmpEntries = new ArrayList<Object>();
		for( IPropertySheetEntry obj : filteredEntries ) {
			tmpEntries.add(obj);
		}
