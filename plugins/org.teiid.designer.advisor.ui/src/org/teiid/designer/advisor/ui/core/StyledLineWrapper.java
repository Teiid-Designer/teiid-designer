...
public class StyledLineWrapper implements StyledTextContent {
    ...
    public StyledLineWrapper( String text,
        ...
        if (text == null || text.length() == 0) text = " "; // use one blank space //$NON-NLS-1$
        setText(text);
    }
    ...
    public String getTextRange( int start,
        ...
        if (l1 == l2) return getLine(l1).substring(start - getOffsetAtLine(l1), end - start);
    ...
    /**
     * Create an array of lines with sytles stripped off. Each lines is at most MAX_LINE_LENGTH characters.
     */
     ...
     */
    private int getLineBreak( String line,
                              GC gc ) {
    ...
    /**
     * Creates all the (bold) style ranges for the text. It is assumed that the text has been split across lines.
     */
