...
public class MappingDiagramUtil {
    ...
    public static Diagram getCoarseMappingDiagram(EObject documentTreeRootEObject) {
        ...
        if( modelResource != null ) {
            try {
                ...
                String message = UiConstants.Util.getString("ShowMappingDiagramAction.getDiagramsError", modelResource.toString());  //$NON-NLS-1$
            ...
            if( iter != null ) {
                ...
                while( iter.hasNext() ) {
                    ...
                    if( nextDiagram.getType() != null &&
                        nextDiagram.getType().equals(PluginConstants.MAPPING_DIAGRAM_TYPE_ID))
                        return nextDiagram;
                }
            }
            ...
            try {
                ...
                Diagram depDiagram = modelResource.getModelDiagrams().createNewDiagram(documentTreeRootEObject, persist); // Do Not persist this diagram.
                depDiagram.setType(PluginConstants.MAPPING_DIAGRAM_TYPE_ID);
                succeeded = true;
                return depDiagram;
            } catch (ModelWorkspaceException mwe) {
                String message = UiConstants.Util.getString("ShowMappingDiagramAction.createMappingDiagramError", modelResource.toString());  //$NON-NLS-1$
    ...
    public static Diagram getCoarseMappingDiagram(Diagram detailedMappingDiagram) {
        ...
        if( documentTreeRoot != null )
            return getCoarseMappingDiagram(documentTreeRoot);
        return null;
    }
    ...
    public static boolean isMappingSqlTable(final Object input) {
        ...
        if( input instanceof EObject && 
            TransformationHelper.isVirtualSqlTable(input) &&
            input instanceof MappingClass ) {
            return true;
        }
    ...
    public static boolean isMappingDocument(final Object input) {
        boolean result = false;
        if( input instanceof EObject && 
            ModelMapperFactory.isTreeRoot((EObject)input) ) {
            return true;
        }
    ...
    public static EObject getMappingDocument(final Object input) {
        ...
        if( input instanceof EObject ) {
            ...
            if( parent != null && parent instanceof EObject ) {
                if( isMappingDocument(parent) )
                    documentEObject = (EObject)parent;
                else
                    documentEObject = getMappingDocument(parent);
            }
        }
    ...
    public static void layoutDiagram(final DiagramModelNode diagramNode, boolean layoutChildren) {
        // Get current editor and check for matching diagram node.
        DiagramEditor editor = DiagramEditorUtil.getVisibleDiagramEditor();
        if( diagramNode != null && editor != null && editor.getCurrentModel() != null &&
            editor.getCurrentModel().getModelObject().equals(diagramNode.getModelObject())) {
    ...
    public static void hiliteUnconnectedExtents(final DiagramModelNode diagramNode) {
		...
		DiagramEditor editor = DiagramEditorUtil.getVisibleDiagramEditor();
		if( diagramNode != null && 
            diagramNode.getModelObject() != null &&
			editor != null && editor.getCurrentModel() != null 
        ...
        while( iter.hasNext() && inputSetEP == null ) {
            ...
            if( nextObj instanceof DiagramEditPart ) {
                ...
                if( nextDEP.getModelObject() != null &&
                    TransformationHelper.isSqlInputSet(nextDEP.getModelObject()) )
                    inputSetEP = nextDEP;
            }
        }
    ...
    public static DiagramEditPart getTransformationEditPart(DiagramEditPart detailedDiagramEditPart ) {
        ...
        while( iter.hasNext() && transformationEP == null ) {
            ...
            if( nextObj instanceof DiagramEditPart ) {
                nextDEP = (DiagramEditPart)nextObj;
                if( nextDEP.getModelObject() != null &&
                    TransformationHelper.isTransformationMappingRoot(nextDEP.getModelObject()) )
    ...
    /**
     * This utility method will:
     * 1) Locate visible Diagram Editor
     * 2) Obtain the MappingDocumentController
     * 3) Get the filter from the controller and use to get visible ordered mapping classes 
     * @return
     * @since 4.3
     */
    ...
    public static boolean isDetailedDiagram(Diagram diagram) {
        if( diagram != null && diagram.getType() != null && diagram.getType().equals(PluginConstants.MAPPING_TRANSFORMATION_DIAGRAM_TYPE_ID)) {
            return true;
        }
        return false;
    }
