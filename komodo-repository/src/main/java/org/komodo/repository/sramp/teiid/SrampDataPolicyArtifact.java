...
/**
 * An S-RAMP data policy artifact.
 */
final class SrampDataPolicyArtifact extends DataPolicy implements DataPolicyArtifact, SrampArtifact {
    ...
    /**
     * @param delegate the S-RAMP data policy artifact (cannot be <code>null</code>)
     * @param repository the S-RAMP repository where the artifact is found (cannot be <code>null</code>)
     */
    public SrampDataPolicyArtifact(final ExtendedArtifactType delegate,
                                   final SrampRepository repository) {
        Precondition.notNull(delegate, "delegate"); //$NON-NLS-1$
        Precondition.matchesExactly(delegate.getExtendedType(), DataPolicyArtifact.TYPE.getId(), "data policy artifact type"); //$NON-NLS-1$
    ...
    public Type getArtifactType() {
        return DataPolicyArtifact.TYPE;
    }
