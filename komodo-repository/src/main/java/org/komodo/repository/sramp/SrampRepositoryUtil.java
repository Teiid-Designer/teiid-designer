...
public class SrampRepositoryUtil implements SrampRepositoryConstants {
    ...
    private static final String DERIVED_ARTIFACTS_PATTERN = USER_DEFINED_ARTIFACT_PATH + "[relatedDocument[@uuid = '%s']]"; //$NON-NLS-1$

    private static final String ELEMENT_QUERY_PATTERN = "./%s"; //$NON-NLS-1$

    private static final String UUID_QUERY_PATTERN = ROOT_PATH + "[@uuid = '%s']"; //$NON-NLS-1$
     ...
     */
    public static String buildQuery(final SoaRepository.QuerySettings settings) {
        Precondition.notNull(settings, "settings"); //$NON-NLS-1$

        // construct query path
        String path = USER_DEFINED_ARTIFACT_PATH;
