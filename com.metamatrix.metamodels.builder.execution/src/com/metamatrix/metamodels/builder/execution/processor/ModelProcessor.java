...
/** 
 * ModelsProcessor - executes a query against the provided schema 'ToCreate' table, 
 * using the sqlConnection.  The results of the query are used to generate model Resources.
 */
public class ModelProcessor extends AbstractProcessor {
     ...
     */
	public ModelProcessor(Connection sqlConnection, ModelBuilder modelBuilder, String modelAndSchemaName, ResourceSet resourceSet, 
			              String targetLocStr, MultiStatus status) {
		...
		if(tableNames!=null) {
			for(int i=0; i<PROCESSING_ORDER.length; i++) {
				if(tableNames.contains(PROCESSING_ORDER[i])) {
					if(this.builderDebugEnabled) {
						final String msg = "Counting rows for table: "+PROCESSING_ORDER[i];  //$NON-NLS-1$
				        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
			        }
					...
					if(this.builderDebugEnabled) {
						final String msg = "Skipping Table Row Count: "+PROCESSING_ORDER[i] + ", Not found in model";  //$NON-NLS-1$  //$NON-NLS-2$
				        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
					}
				}
			}
	...
	public IStatus process(IProgressMonitor monitor) {
		...
		if(this.builderDebugEnabled) {
    		final String msg = ">>> Started Processing the Model Table <<<";   //$NON-NLS-1$
	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
        }
		...
		if(tableNames!=null) {
			for(int i=0; i<PROCESSING_ORDER.length; i++) {
				if(tableNames.contains(PROCESSING_ORDER[i])) {
					if(this.builderDebugEnabled) {
						final String msg = "Processing Table: "+PROCESSING_ORDER[i];  //$NON-NLS-1$
		    	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
					}
					...
					if(this.builderDebugEnabled) {
						final String msg = "Skipping Table: "+PROCESSING_ORDER[i] + ", Not found in model";  //$NON-NLS-1$  //$NON-NLS-2$
		    	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
					}
				}
			}
        ...
        if(this.builderDebugEnabled) {
            final String msg = ">>> Finished Processing the Model Tables <<<";  //$NON-NLS-1$
	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
        }
		return this.status;
	}
	...
	private void processToCreateTable(IProgressMonitor monitor) { 
		...
		if(this.builderDebugEnabled) {
	        final String msg = "ToCreate Processing complete";   //$NON-NLS-1$
	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
		}
	}
	...
	private int countToCreateTableRows() { 
		...
		if(this.builderDebugEnabled) {
	        final String msg = "ToCreate Row Count complete";   //$NON-NLS-1$
	        MetamodelBuilderExecutionPlugin.Util.log(IStatus.INFO, msg);
		}
		return tableRows;
	}
