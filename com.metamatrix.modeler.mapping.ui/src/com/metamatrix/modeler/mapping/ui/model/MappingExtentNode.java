...
public class MappingExtentNode extends AbstractDiagramModelNode {
    ...
    public MappingExtentNode(DiagramModelNode diagramModelNode, EObject modelObject, MappingExtent theExtent, boolean isCoarse) {
        ...
        // set Default size here. (i.e. width will remain constant)
//        updateModelForExtent();
        
        setName("M"); //$NON-NLS-1$
        
        if( UiConstants.Util.isDebugEnabled(DebugConstants.MAPPING_DIAGRAM_MODEL_NODE)) { 
            String message = "New Extent Offset = " + getExtent().getOffset() + " Doc Ref = " + getExtent().getDocumentNodeReference();  //$NON-NLS-1$ //$NON-NLS-2$
...
//            System.out.println("[MappingExtentNode.setExtentPositon] iExtentOffset: " + iExtentOffset + " newYOrigin: " + newYOrigin + " newY: " + newY );
	...
	public List getToolTipStrings() {
		...
		if( isOnCoarseMappingDiagram() ) {
            ...
            if ( getExtent().getMappingReference() != null ) {
				String stName = ModelerCore.getModelEditor().getName(getExtent().getMappingReference());
