...
public class UmlAttributeFigure extends LabeledRectangleFigure implements DirectEditFigure {
    ...
    public UmlAttributeFigure(String labelString, ColorPalette colorPalette) {
        super(labelString, true, colorPalette);
        showSelected(false);
    }
    
    
    public UmlAttributeFigure(String labelString, Image icon, ColorPalette colorPalette) {
        super(labelString, icon, true, colorPalette);
        showSelected(false);
    }
    
    public UmlAttributeFigure(String labelString, Font newFont, ColorPalette colorPalette) {
...
//        super.resize();
    ...
    /**
     * Decorate the figure as necessary for the attribute's datatype
     */
    public void updateForType(UmlAttributeNode node) {
        EObject type = DatatypeUtilities.getSqlColumnDatatype(node.getModelObject());
        if ( type != null && ModelerCore.getDatatypeManager(type, true).isEnumeration(type) ) {
            if( enumIcon == null ) {
                enumIcon = new ImageFigure(DiagramUiPlugin.getDefault().getImage(PluginConstants.Images.ENUM_ICON));
                if( enumIcon != null ) {
                    this.add(enumIcon);
                    enumIcon.setLocation(new Point(12, 9));
                    enumIcon.setSize(enumIcon.getPreferredSize());
                }
            }
        } else if( enumIcon != null ) {
            this.remove(enumIcon);
            enumIcon = null;
        }
    }
