...
public class DiagramUiUtilities {
    ...
    public static String getFontString(Font theFont) {
        ...
        firstFont.getHeight();
        String message = 
            "  Font =" + "( " + firstFont.getName() + //$NON-NLS-2$ //$NON-NLS-1$
...
//        }
    ...
    public static void deleteDiagramEntity( final DiagramEntity diagramEntity, final Object txnSource ) {
	...
	public static void deleteDiagramEntities( final List diagramEntities, final Object txnSource ) {
    ...
    public static DiagramModelNode getDiagramModelNode(EObject eObj, DiagramModelNode diagramRootModelNode) {
        ...
        if( currentChildren != null && !currentChildren.isEmpty() ) {
            ...
            while( iter.hasNext() ) {
                ...
                if( childModelNode.getModelObject() != null && 
                    childModelNode.getModelObject() != null ) {
                    if( eObj.equals( childModelNode.getModelObject() ) )
	...
	public static DiagramModelNode getRootDiagramModelNode(DiagramModelNode diagramModelNode) {
        ...
        if( diagramModelNode.getModelObject() != null ) {
        	...
        	while(  rootNode == null && 
        			diagramModelNode.getParent() != null && 
        			diagramModelNode.getModelObject() != null ) {
    ...
    public static boolean diagramContainsEObject(EObject eObj, DiagramModelNode diagramRootModelNode ) {
        if( getDiagramModelNode(eObj, diagramRootModelNode) != null )
            return true;
...
//        diagramEP.getEditPart(targetDiagramModelNode);
    ...
    public static boolean isDiagramObject(EObject eObject) {
        if( eObject instanceof DiagramContainer ||
            eObject instanceof Diagram ||
            eObject instanceof AbstractDiagramEntity )
            return true;
    ...
    public static boolean isNonDrawingDiagramObject(EObject eObject) {
        if( (eObject instanceof DiagramContainer ||
             eObject instanceof Diagram ||
             eObject instanceof DiagramEntity) 
            && !isDrawingObject(eObject) )
            return true;
    ...
    public static boolean isDrawingObject(EObject eObject ) {
        ...
        if( eObject instanceof DiagramEntity ) {
            ...
            if( userType != null ) {
                if( userType.equals(DrawingConstants.Types.ELLIPSE) ||
                    userType.equals(DrawingConstants.Types.NOTE) ||
                    userType.equals(DrawingConstants.Types.RECTANGLE) ||
                    userType.equals(DrawingConstants.Types.TEXT) ) {
                    result = true;
                }
            }
        }
    ...
    public static boolean isPackageDiagram(EObject eObject) {
        if( eObject instanceof Diagram &&
            ...
            ((Diagram)eObject).getType().equals(PluginConstants.PACKAGE_DIAGRAM_TYPE_ID))
            return true;
    ...
    public static EObject getParentClassifier(EObject eObject) {
		...
		if( DiagramUiPlugin.getDiagramAspectManager().isClassifier(eObject) ) {
			...
			if( parentObject != null && parentObject instanceof EObject && DiagramUiPlugin.getDiagramAspectManager().isClassifier((EObject)parentObject ) ) {
...
//                }
    ...
    public static EObject getParentPackage(final Object input) {
        ...
        if( input instanceof EObject ) {
            ...
            if( parent != null && parent instanceof EObject ) {
                if( isStandardUmlPackage((parent)) )
                    packageEObject = (EObject)parent;
                else
                    packageEObject = getParentPackage(parent);
            }
        }
        ...
        while( iter.hasNext() ) {
            ...
            if( nextObject instanceof DiagramModelNode ) {
                ...
                if( diagramNodeIsLinked(nextDiagramNode) ) {
                    if( connectedNodes.get(nextDiagramNode) == null )
                        connectedNodes.put(nextDiagramNode, "x"); //$NON-NLS-1$
                }
            }
        }
    ...
    public static List getAllSourceConnections(final DiagramModelNode parentDiagramModelNode) {
        ...
        while( iter.hasNext() ) {
            ...
            if( nextObject instanceof DiagramModelNode ) {
                nextDiagramNode = (DiagramModelNode)nextObject;
                if( nextDiagramNode.getSourceConnections() != null && 
                    !nextDiagramNode.getSourceConnections().isEmpty() ) {
    ...
    public static List getNonConnectedModelNodes(final DiagramModelNode parentDiagramModelNode) {
        ...
        while( iter.hasNext() ) {
            nextObject = iter.next();
            if( nextObject instanceof DiagramModelNode 
                && !(nextObject instanceof AbstractFreeDiagramModelNode) 
                && !(nextObject instanceof DrawingModelNode)) {
                nextDiagramNode = (DiagramModelNode)nextObject;
                if( !diagramNodeIsLinked(nextDiagramNode) &&
                    nonConnectedNodes.get(nextDiagramNode) == null )
					nonConnectedNodes.put(nextDiagramNode, "x"); //$NON-NLS-1$
            }
        }
    ...
    public static boolean diagramNodeIsLinked(DiagramModelNode diagramModelNode) { 
        if( diagramModelNode.getSourceConnections() != null && 
            !diagramModelNode.getSourceConnections().isEmpty() )
            return true;
            
        if( diagramModelNode.getTargetConnections() != null && 
            !diagramModelNode.getTargetConnections().isEmpty() )
            return true;
        ...
        if( resrc != null ) {
            ...
            if(!errorOccurred) {
                for (int ndx = markers.length;  --ndx >= 0;) {
                    ...
                    boolean useImprovements = UiConstants.Util.isDebugEnabled("treeMappingAdapter"); //$NON-NLS-1$
                    
                    if( useImprovements ) {
                        targetEObject = ModelWorkspaceManager.getModelWorkspaceManager().getMarkerManager().getMarkedEObject(resrc, marker);
                    } else {
                        targetEObject = ModelObjectUtilities.getMarkedEObject(marker);
                    }
                    
                    boolean usable = ((element == targetEObject) ||
                            ModelObjectUtilities.isDescendant(element,
                            targetEObject));
                    if (!usable) {
                        continue;
                    }
                    ...
                    if (severity == IMarker.SEVERITY_ERROR) {
                        icon = DiagramUiPlugin.getDefault().getImageDescriptor(
                                PluginConstants.Images.ERROR_ICON);
                        break;
                    }
                    if (icon == null  &&  severity == 
                            IMarker.SEVERITY_WARNING) {
                        icon = DiagramUiPlugin.getDefault().getImageDescriptor(
                                PluginConstants.Images.WARNING_ICON);
                    }
                }
            }
        }
        ...
        if( resrc != null ) {
            ...
            if(!errorOccurred) {
                for (int ndx = markers.length;  --ndx >= 0;) {
                    ...
                    if (errorState == DiagramUiConstants.NO_ERRORS &&
		...
		while( iter1.hasNext() ) {
			...
			if( nextObject instanceof DiagramModelNode ) {
				...
				if( nextNode.getModelObject() != null &&
					(nextNode.hasErrors() || nextNode.hasWarnings()) ) {
	...
	public static boolean isNestedClassifier(DiagramModelNode targetNode) {
		if( targetNode.getParent() != null && 
			targetNode.getParent() instanceof UmlClassifierContainerNode )
			return true;
	...
	public static boolean isNestedClassifier(DiagramEditPart editPart) {
		if( ((DiagramModelNode)editPart.getModel()).getParent() != null &&
			((DiagramModelNode)editPart.getModel()).getParent() instanceof UmlClassifierContainerNode )
			return true;
	...
	public static boolean isNodeConnected(DiagramModelNode node) {
		if( (node.getSourceConnections() == null ||
			 node.getSourceConnections().isEmpty() ) &&
			(node.getTargetConnections() == null ||
	...
	public static boolean nodesAreConnected(DiagramModelNode sourceNode, DiagramModelNode targetNode) {
		...
		while( iter.hasNext()) {
			...
			if( ((DiagramModelNode)nextConn.getTargetNode()).getModelObject().equals(targetNode.getModelObject()))
				return true;
		}
		...
		while( iter.hasNext()) {
			nextConn = (NodeConnectionModel)iter.next();
			if( ((DiagramModelNode)nextConn.getSourceNode()).getModelObject().equals(targetNode.getModelObject()))
				return true;
		}
	...
	public static boolean hasDiagramEntity(Diagram diagram, EObject eObject) {
		...
		while( iter.hasNext() ) {
			...
			if( someDEMO != null && someDEMO.equals(eObject))
				return true;
		}
    ...
    public static DiagramLink findDiagramLink(Diagram diagram, EObject eObject) {
        ...
        while( iter.hasNext() ) {
            nextDE = (AbstractDiagramEntity)iter.next();
            EObject someDEMO = nextDE.getModelObject();
            if( someDEMO != null && nextDE instanceof DiagramLink && someDEMO.equals(eObject))
	...
	public static boolean getReadOnlyState(Object someObject) {
		...
		if( someObject instanceof Diagram ) {
			...
			if( diagramNode.getModelObject() != null && diagramNode.getModelObject() instanceof Diagram )
