...
public class NamespaceResolver implements IExtensibleURIResolver {
    ...
    public NamespaceResolver() throws Exception {
        ...
        if (!xsdDirectory.exists()) {
            throw new Exception(ModelGeneratorWsdlPlugin.Util.getString("NamespaceResolver.BadDirectory")); //$NON-NLS-1$
        }
        ...
        if (xsdDirectory.canRead()) {
            ...
            for (int i = 0; i < allContents.length; i++) {
                ...
                if (path.endsWith(".xsd")) { //$NON-NLS-1$
        ...
        for (Iterator<String> iter = xsdPaths.iterator(); iter.hasNext();) {
            ...
            namespaceToXsd.put(namespace, "file:" + path); //$NON-NLS-1$
        }
    }

    @Override
    public void resolve( String baseLocation,
                         String publicId,
                         String systemId,
