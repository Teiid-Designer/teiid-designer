...
//  private static final String EXPORT_TOOL_VALUE             = ApplicationInfo.getInstance().getMainComponent().getTitle();// "MetaMatrix MetaData Modeler";
//  private static final String EXPORT_TOOL_VERSION_VALUE     = ApplicationInfo.getInstance().getMainComponent().getReleaseNumber();    // "2.1";
//  private static final String EXPORT_TOOL_BUILD_NUMBER_VALUE= ApplicationInfo.getInstance().getMainComponent().getBuildNumber();
//  private static final String EXPORT_TOOL_BUILD_DATE_VALUE  = ApplicationInfo.getInstance().getMainComponent().getBuildDate();
    // mtkTODO: Use the correct export tool name/version...
    public static final String EXPORT_TOOL_NAME = DdlPlugin.Util.getString("DdlWriterImpl.ExportToolName"); //$NON-NLS-1$
    public static final String EXPORT_TOOL_VERSION = DdlPlugin.Util.getString("DdlWriterImpl.ExportToolVersion"); //$NON-NLS-1$
...
//    }
     ...
     */
    public IStatus write(final Resource emfResource, final String modelName, final String modelFilename,
                         final OutputStream stream, final IProgressMonitor monitor) {
        ArgCheck.isNotNull(emfResource);
        ArgCheck.isNotNull(stream);
        ArgCheck.isNotNull(modelName);
        final ModelContents contents = emfResource instanceof EmfResource ?
                                       ((EmfResource)emfResource).getModelContents() :
                                       new ModelContents(emfResource);
     ...
     */
    public IStatus write(final ModelResource model, final OutputStream stream,
     ...
     */
    public IStatus write(final ModelWorkspaceSelections selections, final OutputStream stream, 
                         final IProgressMonitor monitor) {
