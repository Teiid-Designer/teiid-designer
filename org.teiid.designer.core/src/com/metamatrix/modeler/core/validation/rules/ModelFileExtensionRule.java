...
public class ModelFileExtensionRule implements ResourceValidationRule {
    ...
    /**
     * Return an IStatus indicating if the specified file name is considered to have a valid file extension.
     * The extension is considered invalid if it matches one of the well-known file extensions for the modeler
     * (e.g. ".xmi", ".xsd", ".vdb") but the extension is not lower-case.  There are a number of places in
     * the modeler and console code that assumes lower-case extensions.  Because of this, a mixed or upper-case
     * extension could produce problems which is the reason for the validation rule.  This method will return
     * an OK status if the specified file name has no extension, is not one of the well-known extensions, is a
     * well-known extension with the correct case.
    ...
    private static boolean isReservedInvalidModelName(String name) {
    	...
    	if(ResourceNameUtil.METAMATRIX_VDBMANIFESTMODEL_NAME.equals(name) ||
    	   ResourceNameUtil.XMLSCHEMA_INSTANCE_NAME.equals(name) ||
    	   ResourceNameUtil.SIMPLEDATATYPES_INSTANCE_NAME.equals(name)) {
