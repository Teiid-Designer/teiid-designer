...
public class Uml2PropertyUmlAspect extends AbstractUml2NamedElementUmlAspect implements UmlProperty {
    ...
    public IStatus setSignature( Object eObject,
        ...
        try {
            ...
            return new Status(IStatus.ERROR, Uml2Plugin.PLUGIN_ID, 0, e.getMessage(), e);
        }
    ...
    /**
     * An attribute can be shown as a text string that can be parsed into the the various properties of an attribute. The basic
     * syntax is (with optional parts shown in braces): [visibility][/]name[:type][multiplicity][=default][{property-string}] In
     * the following bullets, each of these parts is described: <li>visibility is a visibility symbol such as +, -, #, or ~. /
     * means the attribute is derived. name is the name of the attribute. type identifies a classifier that is the attribute�s
     * type. multiplicity shows the attribute�s multiplicity in square brackets. The term may be omitted when a multiplicity of 1
     * (exactly one) is to be assumed. default is an expression for the default value or values of the attribute. property-string
     * indicates property values that apply to the attribute. The property string is optional (the braces are omitted if no
     * properties are specified).</li>
     * <p>
     * The following property strings can be applied to an attribute: {readOnly}, {union}, {subsets <property-name>}, {redefines
     * <property-name>}, {ordered}, {bag}, {seq} or {sequence}, and {composite}. An attribute with the same name as an attribute
     * that would have been inherited is interpreted to be a redefinition, without the need for a {redefines <x>} property string.
     * Note that a redefined attribute is not inherited into a namespace where it is redefined, so its name can be reused in the
     * featuring classifier, either for the redefining attribute, or alternately for some other attribute.
     * </p>
     * <p>
     * If the property represents an association end The following property strings can be applied:
     * <li>{subsets <property-name>} to show that the end is a subset of the property called <property-name>. {redefined
     * <end-name>} to show that the end redefines the one named <end-name>. {union} to show that the end is derived by being the
     * union of its subsets. {ordered} to show that the end represents an ordered set. {bag} to show that the end represents a
     * collection that permits the same element to appear more than once. {sequence} or {seq} to show that the end represents a
     * sequence (an ordered bag).</li>
     * </p>
     * 
    ...
    /**
     * An attribute can be shown as a text string that can be parsed into the the various properties of an attribute. The basic
     * syntax is (with optional parts shown in braces): [visibility][/]name[:type][multiplicity][=default][{property-string}] The
