...
public class Uml2OperationUmlAspect extends AbstractUml2NamedElementUmlAspect implements UmlOperation {
	...
	protected void appendName(final Operation umlOperation, final StringBuffer sb) {
        super.appendName(umlOperation,sb);
	}

	protected void appendReturnType(final Operation operation, final StringBuffer result, final boolean includePrefix) {
...
//		}
	...
	protected void appendVisibility(final Operation operation, final StringBuffer result) {
	...
	protected void appendParameters(final Operation operation, final StringBuffer result) {
    ...
    protected List getOutputParameters(final Operation operation) {
        final List params = operation.getOwnedParameters();
        final List result = new ArrayList(params.size());
        for (Iterator iter = params.iterator(); iter.hasNext();) {
            Parameter p = (Parameter) iter.next();
            if (p.getDirection() == ParameterDirectionKind.OUT_LITERAL || p.getDirection() == ParameterDirectionKind.INOUT_LITERAL) {
                result.add(p);
            }
