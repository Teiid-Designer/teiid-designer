...
public class GenerateWebServiceModelAction2 extends SortableSelectionAction implements IInternalUiConstants {
    ...
    public void run() {
        ...
        if( selection != null && !selection.isEmpty() && isDocumentOrRootSelected(selection)) {
            ...
            XmlDocumentSelectorDialog xmlDialog 
                = new XmlDocumentSelectorDialog( WebServiceUiPlugin.getDefault().getCurrentWorkbenchWindow().getShell() , 
            ...
            if ( xmlDialog.getReturnCode() == Window.OK ) {
                ...
                if ( oSelectedObjects.length == 1  ) {
                    if( oSelectedObjects[ 0 ] instanceof XmlDocument || oSelectedObjects[ 0 ] instanceof XmlRoot) {
                        ...
                        // ERROR !!!!!
                        String title = 
                            IInternalUiConstants.UTIL.getString("GenerateWebServiceModelAction.selectionError.title"); //$NON-NLS-1$
                        String dlgMsg = 
                            IInternalUiConstants.UTIL.getString("GenerateWebServiceModelAction.selectionError.wrongType"); //$NON-NLS-1$
                    ...
                    String title = 
                        IInternalUiConstants.UTIL.getString("GenerateWebServiceModelAction.selectionError.title"); //$NON-NLS-1$
                    String dlgMsg = 
                        IInternalUiConstants.UTIL.getString("GenerateWebServiceModelAction.selectionError.emptySelection"); //$NON-NLS-1$
                    MessageDialog.openError(iww.getShell(), title, dlgMsg);
                    notifyResult(false);
                    return;
                }
            ...
            IViewReference ivr = 
                iww.getActivePage().findViewReference(ProductCustomizerMgr.getInstance().getProductCharacteristics().getPrimaryNavigationViewId());
            String viewTitle;
            if (ivr != null) {
