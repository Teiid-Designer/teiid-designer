...
/**<p>
 * This is the interface to the MetaMatrix Debug Facility, which provides debugging capabilities for all MetaMatrix plug-ins
 * <em>and</em> third-party extensions to MetaMatrix extension points.  Debug code is enable or disabled via a set of Boolean
 * properties, which are generally associated with particular "contexts", or application processes.  These properties can be
 * modified in one of three ways:
 * </p><ul><li>
 * Changing the debug settings in a client MetaMatrix application's
 * {@link com.metamatrix.internal.ui.preferences.DebugPreferencePage debug preference page},
 * </li><li>
 * Programatically via the static methods in this interface's sole {@link DebuggerImpl implementation}*, or
 * </li><li>
 * By manually editing the {@link Constants#FILENAME debug properties file} that resides in the
 * {@link CoreModelerPlugin#PLUGIN_ID MetaMatrix Core} plug-in's state folder.
 * </li></u><p>
 ...
 * available.  In addition to contexts specific to tracing, this feature offers class-based tracing, which provides the abililty
 * to enable or disable trace code at individual plug-in, package, and class levels.
 * </p><p>
 ...
 * file. 
 * </li><li>
 * Information for the {@link Constants#PRINT_THREAD_PROPERTY current thread} can optionally be prefixed to each debug message.
 * </li><li>
 * A {@link Constants#PRINT_TIMESTAMP_PROPERTY timestamp} can optionally be prefixed to each debug message.
 * </li><li>
 * Trace messages can optionally be {@link Constants#INDENT_MESSAGE_PROPERTY indented} to highlight the hierarchy of method calls
 * (Only applies to class-level tracing).
 * </li></u><p>
 ...
 * must not be used within the names of any contexts defined by plug-ins.  With the exception of the
 * {@link Constants@TRACE_FILTERS_PROPERTY trace filters} property, all reserved properties are defined in the
 * {@link CoreModelerPlugin#PLUGIN_ID MetaMatrix Core} plug-in.  All property names are specified in a hierarchical manner, such that
 * each name includes the names of its ancestors, in order, separated by a special
 * {@link Constants#SEPARATOR name separation character}.  Note, however, that the {@link Constants#DEBUG_ENABLED_PROPERTY root}
 * property that controls the enabling of the entire Debug Facility, is an exception to this rule, and does not appear within
 ...
 * file.
 * </p><ul><li>
 * The actual property, whose value represents the default value of a context if one is not explicitly set.
 * </li><li>
 * A description property that describes the context or group.  This is the only property that may also be specified for traced
 * classes.  A description property's name must exactly match the name of the actual property, followed by the reserved
 * {@link Constants#DESCRIPTION_SUFFIX description suffix}.
 * </li><li><p>
 * An allowed values property that describes the type of values that a context may contain.  An allowed values property's name
 * must exactly match the name of the actual property, followed by the reserved
 * {@link Constants#VALUES_SUFFIX allowed values suffix}.  This property's value may be a set of strings, separated by either a
 * {@link Constants#LIST_DELIMITER list delimiter}, which signifies the actual property may contain multiple values, or a
 * {@link Constants#CHOICE_DELIMITER choice delimiter}, which indicates the actual value may contain only one of the string values
 * listed.  A {@link Constants#NONE_VALUE special reserved value} may be specified if no values are allowed for the actual
 * property.  For sets of choices, two additional, mutually-exclusive reserved values may be specified to indicate that either
 * {@link Debugger#ANY_VALUE any value} or, more specifically, a {@link Constants#FILE_VALUE file name} may be included as a
 * choice.  For exammple:
 * </p><p>
 ...
 * which resides in the {@link CoreModelerPlugin#PLUGIN_ID MetaMatrix Core} plug-in's runtime state folder.  This file may be modified
...
public interface Debugger {
    ...
    interface Constants extends FileUtils.Constants,
    ...
    /**<p>
     * Returns whether tracing is enabled for the specified context.  For this method to return true, all of the context's
     * ancestor contexts must also be enabled.
     * </p>
    ...
    /**<p>
     * Returns whether tracing is enabled for the specified class.  For this method to return true, the class' package must also
     * be enabled.  Only intended for use within static methods.
     * </p>
    ...
    /**<p>
     * Prints the specified message to the currently set debug output stream, prepending the context.  Intended for use
     * when it has already been determined that tracing is enabled for the class in an outer block of code, such as within a
     ...
     * @since 4.0
     */
    void print(String context, String message);

    /**<p>
     * Prints the specified message to the currently set debug output stream, prepending the class name, if tracing is enabled,
     * but regardless of whether tracing is {@link #isTraceEnabled(Class) enabled} for the specified class.  Intended for use
     * when it has already been determined that tracing is enabled for the class in an outer block of code, such as within a
     ...
     * @since 4.0
     */
    void print(Class clazz, String message);
     ...
     * @since 4.0
     */
    void print(Object object, String message);

    /**<p>
     * Specialized version of {@link #print(String, String)}, intended for use at the beginning of a block of code (e.g.,
     * a method), that prepends the specified message with an indicator that the block has been entered.  If indentation is
     * enabled, increments the current indent level for the current thread after printing the specified message.  Must be used in
     * conjuntion with {@link #printExited(String, String)} for indented entered-exited message pairs to align.
     * </p>
     * @param context A trace context.
     * @param message A message to be printed.
     * @since 4.0
     */
    void printEntered(String context, String message);

    /**<p>
     * Specialized version of {@link #print(Class, String)}, intended for use at the beginning of a block of code (e.g.,
     * a method), that prepends the specified message with an indicator that the block has been entered.  If indentation is
     * enabled, increments the current indent level for the current thread after printing the specified message.  Must be used in
     * conjuntion with {@link #printExited(Class, String)} for indented entered-exited message pairs to align.  Only intended for
     * use within static methods.
     * </p>
     * @param clazz   A class to trace.
     * @param message A message to be printed.
     * @since 4.0
     */
    void printEntered(Class clazz, String message);
     ...
     * @since 4.0
     */
    void printEntered(Object object, String message);

    /**<p> <p>
     * Specialized version of {@link #print(String, String)}, intended for use at the end of a block of code (e.g., a method),
     * that prepends the specified message with an indicator that the block has been exited.  If indentation is enabled,
     * decrements the current indent level for the current thread before printing the specified message.  Must be used in
     * conjuntion with {@link #printEntered(String, String)} for indented entered-exited message pairs to align.
     * </p><p>
     ...
     * @since 4.0
     */
    void printExited(String context, String message);

    /**<p> <p>
     * Specialized version of {@link #print(Class, String)}, intended for use at the end of a block of code (e.g., a method),
     * that prepends the specified message with an indicator that the block has been exited.  If indentation is enabled,
     * decrements the current indent level for the current thread before printing the specified message.  Must be used in
     * conjuntion with {@link #printEntered(Class, String)} for indented entered-exited message pairs to align.  Only intended for
     * use within static methods.
     * </p><p>
     * Note, it is good practice to place calls to this method within the <code>finally</code> block of a try-finally statement
     * surrounding the code block being debugged, to ensure this method is executed regardless of thrown exceptions or internal
     ...
     * @since 4.0
     */
    void printExited(Class clazz, String message);
     ...
     * @since 4.0
     */
    void printExited(Object object, String message);
    
    /**
     * Marks a beginning point in time for the specified context identifier. A log entry marking the start time
     * and context ID is entered if the context is enabled.
     * @param theContextId the context identifier
     */
    void start(String theContextId);
    
    /**
     * Marks a beginning point in time for the specified timer identifier. A log entry marking the start time
     * and timer ID is entered only if the specified context is enabled.
     * @param theTimerId the timer identifier
     * @param theContextId the context identifier
     */
    void start(String theTimerId, String theContextId);
    
    /**
     * Marks a beginning point in time for the specified timer identifier. A log entry marking the start time
     * and timer ID is entered only if the specified context is enabled. Information objects are added to the
     * message by using their string representations.
     * @param theTimerId the timer identifier
     * @param theContextId the context identifier
     * @param theInfo the objects that add additional information to the log message (can be <code>null</code>)
     */
    void start(String theTimerId, String theContextId, Object[] theInfo);
    
    /**
     * Marks the ending point in time for the specified context identifier. A log entry marking the total time
     * consumed for this context is entered.
     * @param theContextId the context identifier
     */
    void stop(String theContextId);

    /**
     * Marks the ending point in time for the specified timer identifier. A log entry marking the total time
     * consumed for this timer is entered if the specified context is enabled.
     * @param theTimerId the timer identifier
     * @param theContextId the specified context
     */
    void stop(String theTimerId, String theContextId);

    /**
     * Marks the ending point in time for the specified timer identifier. A log entry marking the total time
     * consumed for this timer is entered if the specified context is enabled. Information objects are added
     * to the message by using their string representations.
     * @param theTimerId the timer identifier
     * @param theContextId the context identifier
     * @param theInfo the objects that add additional information to the log message
     */
    void stop(String theTimerId, String theContextId, Object[] theInfo);
     ...
     * @since 4.0
     */
    void trace(Object object, String message);
     ...
     * @since 4.0
     */
    void traceEntered(String context, String message);
     ...
     * @since 4.0
     */
    void traceEntered(Class clazz, String message);
     ...
     * @since 4.0
     */
    void traceEntered(Object object, String message);
     ...
     * @since 4.0
     */
    void traceExited(String context, String message);
     ...
     * @since 4.0
     */
    void traceExited(Class clazz, String message);
     ...
     * @since 4.0
     */
    void traceExited(Object object, String message);
}
