...
/**
 * This ObjectMatcher class can be used to find the {@link EObject model object}
 * that best matches a set of {@link JdbcNode JdbcNode database objects}.  Generally,
 * a single instance of this class is created and used to match all of the
 * JdbcNode instances to the {@link EObject objects} in a model, but
 * the {@link #findBestMatches(List, List)} method is invoked once with
 * the children from each model object and the children from the corresponding
 * database object.  This is because the {@link #findBestMatches(List, List)}
 * method only compares the name (rather than the full name or path to the object).
 */
public class ObjectMatcher {
    
    /**
     * Default implementation of {@link MatchValueProvider} that simply performs a 
     * {@link Object#toString() toString()} on the supplied object.
     */
    ...
    /**
     * Implementation of {@link MatchValueProvider} that returns the name of a {@link JdbcNode} object.
     * This method assumes the supplied object is an instance of {@link JdbcNode}.
     */
    ...
    /**
     * Implementation of {@link MatchValueProvider} that returns the name of a {@link RelationalEntity} object.
     * This method assumes the supplied object is an instance of {@link RelationalEntity}.
     */
    ...
    /**
     * Find the best matches between the supplied lists of {@link JdbcNode} instances and
     * {@link RelationalEntity} instances.  Each time a match between a JdbcNode and
     * RelationalEntity, the pair are inserted into the 
     * {@link #getDestination() destination map} with the JdbcNode as the key and the
     * RelationalEntity as the value.
     * <p>
     * Any JdbcNode instances for which no matching RelationalEntity can be found
     * are placed in the {@link #getUnmatchedNodes() unmatched nodes} list.  Similarly,
     * any RelationalEntity instances for which no matching JdbcNode cannot be found
     * are placed in the {@link #getUnmatchedNodes() unmatched nodes} list.
     * </p>
     * <p>
     ...
     */
    public void findBestMatches( final List jdbcNodes, final List modelObjects ) {
    ...
    /**
     * Method used to determine whether the supplied {@link JdbcNode JdbcNode object}
     * has a type that matches the supplied {@link RelationalEntity model object}.
     * @param node
     * @param modelObject
     * @return
     */
    protected boolean isMatchingType( final Object node, final Object modelObject ) {
    ...
    protected void process( final boolean caseSensitive, final List nodes, final List objs, 
                            final List unmatchedNodes, final List unmatchedObjs ) {
        ...
        while (iter.hasNext()) {
            ...
            if ( obj != null /*&& obj instanceof RelationalEntity*/ ) {
                final String name = caseSensitive ?
                                    this.modelObjectNameProvider.getMatchValue(obj) :
                                    this.modelObjectNameProvider.getMatchValue(obj).toUpperCase();
        ...
        while (nodeIter.hasNext()) {
            ...
            final String name = caseSensitive ?
                                this.jdbcNodeNameProvider.getMatchValue(node) :
                                this.jdbcNodeNameProvider.getMatchValue(node).toUpperCase();
