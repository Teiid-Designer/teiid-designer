...
public class PluginManager {
    ...
    public synchronized void sessionStarted( SessionTreeNode sessionNode ) {
        ...
        for (Iterator it = _sessionPlugins.iterator(); it.hasNext();) {
            ...
            try {
                ...
                String msg = "Error occured in IPlugin.sessionStarted() for " + spi.getPlugin().getDescriptiveName(); //$NON-NLS-1$
        ...
        if (plugins != null) {
            for (Iterator it = plugins.iterator(); it.hasNext();) {
                ...
                try {
                    ...
                    String msg = "Error occured in IPlugin.sessionEnding() for " + spi.getPlugin().getDescriptiveName(); //$NON-NLS-1$
                    SQLExplorerPlugin.error(msg, th);
                    // _app.showErrorDialog(msg, th);
                }
            }
    ...
    public IDbModel[] getSchemaAddedTypes( SchemaNode schemaNode,
     ...
     * @return
     */
    public IAction[] getTypeActions( SessionTreeNode sessionNode,
                                     IDbModel node,
                                     TreeViewer tv ) {
        List plugins = (List)_activeSessions.get(sessionNode.getIdentifier());
     ...
     * @return
     */
    public IActivablePanel[] getAddedPanels( SessionTreeNode sessionNode,
                                             IDbModel node ) {
        List plugins = (List)_activeSessions.get(sessionNode.getIdentifier());
