...
/**
 * This caches QueryMetadataInterface implementations for all vdbs, each implementation has access to metadata for a given vdb and
 * the system vdb.
 * 
 * @since 4.2
 */
public class QueryMetadataCache {
    ...
    public QueryMetadataCache( final URL systemVdbUrl ) throws MetaMatrixComponentException {
        try {
            ...
            throw new MetaMatrixComponentException(
                                                   e,
                                                   DQPPlugin.Util.getString("QueryMetadataCache.Failed_creating_Runtime_Index_Selector._4", CoreConstants.SYSTEM_VDB)); //$NON-NLS-1$
        }
    }
     ...
     */
    public IndexSelector getCompositeSelector( final String vdbName,
    ...
    public IObjectSource getCompositeMetadataObjectSource( String vdbName,
                                                           String vdbVersion,
    ...
    public QueryMetadataInterface lookupMetadata( final String vdbName,
                                                  final String vdbVersion,
                                                  final InputStream vdbContent ) {
        QueryMetadataInterfaceHolder qmiHolder = createMetadataHolder(vdbName, vdbVersion);
        return qmiHolder.getQueryMetadataInteface(vdbContent);
    }

    private QueryMetadataInterfaceHolder createMetadataHolder( final String vdbName,
                                                               final String vdbVersion ) {
        assertIsValidCache();
        VDBKey vdbID = toVdbID(vdbName, vdbVersion);
    ...
    /**
     * There a QMI holder per vdb, the holder shields metadata lookups by differrent thereads for differrent vdbs from having to
     * wait for metadata being loaded for a single vdb. If there are 2 threads for 2 vdbs, each thread can have their own instance
     * of the holder and load their own metadata without having to wait for other thread to finish.
     * 
     * @since 4.2
     */
    protected class QueryMetadataInterfaceHolder {
        ...
        protected QueryMetadataInterfaceHolder( VDBKey vdbId,
                                                String vdbName ) {
            this.vdbId = vdbId;
        ...
        protected synchronized QueryMetadataInterface getQueryMetadataInteface( final byte[] vdbContent ) {
            if (this.qmi == null) {
        ...
        protected synchronized QueryMetadataInterface getQueryMetadataInteface( InputStream vdbContent ) {
            if (this.qmi == null) {
                this.qmi = QueryMetadataCache.this.loadMetadata(this.vdbId, getRuntimeIndexSelector(this.vdbName, vdbContent));
            }
            return this.qmi;
    ...
    QueryMetadataInterface loadMetadata( final VDBKey vdbID,
                                         final RuntimeIndexSelector runtimeSelector ) {
        // check cache status
        assertIsValidCache();
        ...
        // if (false) {
        // result = new TimingMetadataInterface( result, ServerPerformance.METADATA,
        // ConnectorManagerImpl.getPerformanceStatisticsSource() );
        // }
        return result;
    }

    RuntimeIndexSelector getRuntimeIndexSelector( final String vdbName,
                                                  byte[] vdbContents ) {
        ...
        try {
            ...
            throw new MetaMatrixRuntimeException(
                                                 e,
                                                 DQPPlugin.Util.getString("QueryMetadataCache.Failed_creating_Runtime_Index_Selector._4", vdbName)); //$NON-NLS-1$
        }
        return runtimeSelector;
    }

    RuntimeIndexSelector getRuntimeIndexSelector( final String vdbName,
                                                  InputStream vdbContents ) {
        RuntimeIndexSelector runtimeSelector = null;
        try {
            String vdbFileName = vdbName + '.' + ModelFileUtil.EXTENSION_VDB;
            runtimeSelector = new RuntimeIndexSelector(vdbFileName, vdbContents);
    ...
    private RuntimeIndexSelector getRuntimeIndexSelector( final URL path ) {
        ...
        try {
            ...
            throw new MetaMatrixRuntimeException(
                                                 e,
                                                 DQPPlugin.Util.getString("QueryMetadataCache.Failed_creating_Runtime_Index_Selector._4", path)); //$NON-NLS-1$
        }
        return runtimeSelector;
    }
     ...
     */
    public void removeFromCache( final String vdbName,
                                 final String vdbVersion ) {
        LogManager.logTrace(LogConstants.CTX_DQP,
                            new Object[] {"QueryMetadataCache Removing vdb from cache", vdbName, vdbVersion}); //$NON-NLS-1$ 
     ...
     */
    private VDBKey toVdbID( final String vdbName,
                            final String vdbVersion ) {
     ...
     */
    public QueryMetadataInterface testLoadMetadata( final String vdbName,
                                                    final String vdbVersion,
                                                    final String filePath ) throws MetaMatrixComponentException {
        VDBKey vdbID = toVdbID(vdbName, vdbVersion);
        try {
