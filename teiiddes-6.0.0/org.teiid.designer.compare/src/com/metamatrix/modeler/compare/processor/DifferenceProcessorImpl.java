...
public class DifferenceProcessorImpl implements DifferenceProcessor {
    ...
    /**
     * Construct a DifferenceProcessor for a single model.  This form of the constructor is used
     * to signal that there are no differences to be computed, but that a difference report
     * should be generated when asked for.  This is called by the 
     * {@link com.metamatrix.modeler.compare.ModelerComparePlugin#createDifferenceProcessor(ModelResource)}
     ...
     * <p>
     * This processor computes the difference between the two models as if recording 
     * the actions necessary to change the <i>before</i> model into the <i>after</i> model.
     * </p>
     ...
     * 
     */
    public DifferenceProcessorImpl( final ModelSelector beforeSelector, final ModelSelector afterSelector ) {
     ...
     * 
     */
    public DifferenceProcessorImpl( final ModelSelector beforeSelector, final ModelSelector afterSelector, final HashMap mappings) {
        super();
    ...
    public IStatus execute( final IProgressMonitor progressMonitor ) {
        ...
        final int totalWork = AMOUNT_OF_WORK_FOR_LOADING_RESOURCES
                            + AMOUNT_OF_WORK_FOR_MAPPING_RESOURCES
                            + AMOUNT_OF_WORK_FOR_COMUTING_DIFFERENCES 
                            + AMOUNT_OF_WORK_FOR_CREATING_RESULTS;
...
//                    ModelerCore.rollbackTxn();
    ...
    protected IStatus newWarning( final int code, final String msg, final Throwable t ) {
        return new Status(IStatus.WARNING,PLUGINID,code,msg,t);
    }
    protected IStatus newInfo( final int code, final String msg, final Throwable t ) {
        return new Status(IStatus.INFO,PLUGINID,code,msg,t);
    }
    protected IStatus newError( final int code, final String msg, final Throwable t ) {
        return new Status(IStatus.ERROR,PLUGINID,code,msg,t);
    }
    protected IStatus newOk( final int code, final String msg, final Throwable t ) {
     ...
     * @param subtaskPrefix the string for subtasks that can be used to prefix the names of the objects
     * being compared  
     */
    protected Mapping createMapping( final List beforeSiblings, final List afterSiblings,
                                     final List problems,
                                     final IProgressMonitor monitor, final String subtaskPrefix ) {
    ...
    protected DifferenceDescriptor setDescriptor( final Mapping mapping, final DifferenceType type ) {
    ...
    protected boolean computeFeatureDifferences( final DifferenceDescriptor descriptor, 
                                                 final EObject input, final EObject output ) {
        ...
        while (iter.hasNext()) {
            ...
            if ( this.includeInReport(feature) ) {
                ...
                if ( feature instanceof EAttribute || 
                    ...
                    final boolean sameValue = feature.isMany() ?
                                              isEqual(feature,(EList)inputValue,(EList)outputValue) :
                                              isEqual(feature,inputValue,outputValue);
                    if ( !sameValue ) {
     ...
     * @param object2
     */
    protected boolean isEqual( final EStructuralFeature feature, final Object object1, final Object object2) {
        if ( object1 == null ) {
            if ( object2 == null ) {
                // Both are null
                return true;
     ...
     * @param list2
     */
    protected boolean isEqual( final EStructuralFeature feature, final EList list1, final EList list2) {
    ...
    /**
     * Return the list of {@link EObjectMappingAdapter} instances that will be used by this processor
    ...
    /**
     * Helper method to add to the list of {@link EObjectMappingAdapter} instances that will be used 
    ...
    /**
     * Method called to create any {@link EObjectMappingAdapter} instances and add them to those
     * already in the list.
     */
