...
public class AddModelProjectSetAction
    implements IActionDelegate2, IWorkbenchWindowActionDelegate, IPartListener, IPropertyListener {
    ...
    public void init( IWorkbenchWindow theWindow ) {
        ...
        // when Eclipse starts and there are editors open, a part event is not
        // received for that
        // editor. no partActivated or partOpened event is received. this is a
        // workaround for that.
        ...
        // need to receive events on all parts. if part is an editor will
        // register
    ...
    public void selectionChanged( IAction theAction,
        ...
        // need to process these since the plugin manifest for this action may
        // enable on selection.
    ...
    public void run( IAction theAction ) {
        IImportWizard wizard = new ImportModelerProjectSetWizard();
        wizard.init(UiPlugin.getDefault().getWorkbench(), new StructuredSelection());
        wizard.addPages();
        WidgetFactory.createWizardDialog(window.getShell(), wizard).open();
