...
/**
 * This Utility class is designed to provide access to specific model information from any Modeler UI plugin
 *
 * If at all possible, the method calls to access <code>ModelType</code> and primary metamodel URI attempt to use the <code>XmiHeader</code> information
 * via an <code>IResource</code>. This insures a light-weight check so <code>ModelResource</code>s don't get loaded needlessly/repeatedly.
 *
 * See <code>PdeTestModelIdentifier</code>
 *
 * @since 5.0
 */
public abstract class ModelIdentifier implements UiConstants {
    ...
    public static final String      UML_MODEL_URI       = "http://www.eclipse.org/uml2/2.1.0/UML"; //$NON-NLS-1$
    ...
    /**
     * Indicates if the specified <code>IResource</code> is a source model. A source model is a relational,
     * physical model.
     * @param theResource the resource being checked (may not be <code>null</code>)
    ...
    /**
     * Indicates if the specified <code>ModelResource</code> is a source model. A source model is a relational,
     * physical model.
     * @param theResource the resource being checked (may not be <code>null</code>)
     * @return <code>true</code> if a source model; <code>false</code> otherwise.
     * @since 5.0
    ...
    public static Image getImage(int modelID) {
        ...
        switch(modelID) {
            case RELATIONAL_SOURCE_MODEL_ID:               {  modelImage = UiPlugin.getDefault().getImage(RELATIONAL_SOURCE_MODEL_IMAGE_ID); } break;

            case RELATIONAL_VIEW_MODEL_ID:                 {  modelImage = UiPlugin.getDefault().getImage(RELATIONAL_VIEW_MODEL_IMAGE_ID); } break;

            case XML_VIEW_MODEL_ID:             {  modelImage = UiPlugin.getDefault().getImage(XML_VIEW_MODEL_IMAGE_ID); } break;

            case EXTENSION_MODEL_ID:            {  modelImage = UiPlugin.getDefault().getImage(EXTENSION_MODEL_IMAGE_ID); } break;

            case FUNCTION_MODEL_ID:             {  modelImage = UiPlugin.getDefault().getImage(FUNCTION_MODEL_IMAGE_ID); } break;

            case XML_SCHEMA_MODEL_ID:           {  modelImage = UiPlugin.getDefault().getImage(XML_SCHEMA_MODEL_IMAGE_ID); } break;

            case LOGICAL_MODEL_ID:              {  modelImage = UiPlugin.getDefault().getImage(LOGICAL_MODEL_IMAGE_ID); } break;

            case WEB_SERVICES_VIEW_MODEL_ID:    {  modelImage = UiPlugin.getDefault().getImage(WEB_SERVICES_VIEW_MODEL_IMAGE_ID); } break;

            case XML_SERVICE_SOURCE_MODEL_ID:   {  modelImage = UiPlugin.getDefault().getImage(XML_SERVICE_SOURCE_MODEL_IMAGE_ID); } break;

            case XML_SERVICE_VIEW_MODEL_ID:     {  modelImage = UiPlugin.getDefault().getImage(XML_SERVICE_VIEW_MODEL_IMAGE_ID); } break;

            case RELATIONSHIP_MODEL_ID:         {  modelImage = UiPlugin.getDefault().getImage(RELATIONSHIP_MODEL_IMAGE_ID); } break;

            case PEOPLE_MODEL_ID:               {  modelImage = UiPlugin.getDefault().getImage(PEOPLE_MODEL_IMAGE_ID); } break;

            case UML_MODEL_ID:                  {  modelImage = UiPlugin.getDefault().getImage(UML_MODEL_IMAGE_ID); } break;

            case XML_MESSAGE_STRUCTURE_MODEL_ID: {  modelImage = UiPlugin.getDefault().getImage(XML_MESSAGE_STRUCTURE_MODEL_IMAGE_ID); } break;
